...
function Newer(options) {
  ...
  if (options.extra) {
    ...
    for (var i = 0; i < options.extra.length; ++i) {
      extraFiles.push(globAsync(options.extra[i]));
    }
    this._extraStats = Promise.all(extraFiles)
      .then(function(fileArrays) {
        ...
        var allFiles = [];
        var j;
        for (j = 0; j < fileArrays.length; ++j) {
          allFiles = allFiles.concat(fileArrays[j]);
        }
        var extraStats = [];
        for (j = 0; j < allFiles.length; ++j) {
          extraStats.push(statAsync(allFiles[j]));
        }
    ...
    // Prevent unhandled rejection (error is re-caught in _transform via Promise.all)
    this._extraStats.catch(function() {});
  }
}
...
Newer.prototype._transform = function(srcFile, encoding, done) {
  ...
  Promise.all([this._destStats, this._extraStats])
    .then(function(results) {
      var destStats = results[0];

      resolvedExtra = results[1];
      if ((destStats && destStats.isDirectory()) || self._ext || self._map) {
        ...
        var destFileJoined = self._dest
          ...
          : destFileRelative;
        return statAsync(destFileJoined).then(function(stat) {
          return [stat, resolvedExtra];
        });
      } else {
        ...
        return [destStats, resolvedExtra];
      }
    })
    .catch(function(err) {
      if (err.code === 'ENOENT') {
        // dest file or directory doesn't exist, pass through all
        return [null, resolvedExtra];
      } else {
        // unexpected error
        throw err;
      }
    ...
    .then(function(results) {
      var destFileStats = results[0];
      var extraFileStats = results[1];
      ...
      if (
        extraFileStats &&
        (!destFileStats || extraFileStats[timestamp] > destFileStats[timestamp])
      ) {
        newer = true;
      }
      if (self._all) {
