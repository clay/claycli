...
function get(url, options = {}) {
  ...
  return h(send(url, {
    ...
    .catch((e) => {
      e.url = url; // capture urls that we error on
      throw e;
    }));
}
...
function recursivelyCheckURI(currentURL, publicURI, options) {
  ...
  possibleUrl = `${possiblePrefix}/_uris/${publicURI}`;

  return send(possibleUrl, {
    method: 'GET',
    headers: options.headers,
    agent: isSSL(possibleUrl) ? agent : null
  }).then((res) => res.text()) // page uri
    .catch(() => {
...
function findURI(url, options = {}) {
  ...
  return h(recursivelyCheckURI(url, publicURI, options, hops));
}
