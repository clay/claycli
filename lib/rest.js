...
function query(url, query, options = {}) {
  ...
  return h(send(url, {
    ...
    if (res instanceof Error) {
      ...
      return res.json().then((obj) => ({ type: 'success', message: pluralize('result', _.get(obj, 'hits.total'), true), details: url, data: _.map(_.get(obj, 'hits.hits', []), (hit) => _.assign(hit._source, { _id: hit._id })), total: _.get(obj, 'hits.total') }));
    }
  }));
}
