...
function resolveKilnPostcssChain(opts = {}) {
  ...
  const recommended = RECOMMENDED_VERSION[postcssMajor];
  const plugins = [];
  const resolved = {};
  ...
  for (const name of KILN_CHAIN_PLUGIN_NAMES) {
    ...
    if (!mod) {
      onIssue(
        ...
        'Kiln plugin styles will build without it — continuing.' +
        (recommendedRange ? ` Install: npm i -D ${name}@${recommendedRange}` : '')
      );
      continue;
    }
    ...
    if (ceiling && postcssMajor != null) {
      const pluginMajor = majorOf(version);
      const requiresPostcss8 = pluginMajor != null && pluginMajor > ceiling;
      const incompatible =
        (postcssMajor <= 7 && requiresPostcss8) ||
        (postcssMajor >= 8 && pluginMajor != null && pluginMajor <= ceiling);

      if (incompatible) {
        onIssue(
          `[clay] ${name}@${version} is incompatible with the resolved ` +
          `postcss@${postcssVersion} (both resolved from ${cwd}) and would ` +
          'throw the first time a Kiln plugin style is compiled. Kiln ' +
          'plugin styles will build without it — continuing.' +
          (recommendedRange ? ` Install: npm i -D ${name}@${recommendedRange}` : '')
        );
        continue;
      }
    ...
    plugins.push(typeof mod === 'function' ? mod(...args) : mod);
