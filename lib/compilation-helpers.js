...
function determinePostCSSPlugins(argv) {
  const config = tryRequire(`${process.cwd()}/claycli.config`);

  if (config && config.styles && config.styles.plugins) {
    if (!Array.isArray(config.styles.plugins)) {
