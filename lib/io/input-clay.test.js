...
describe('input clay', () => {
  ...
  describe('streamAssets', () => {
    const fn = lib.streamAssets,
      ...
      userUrl = 'http://domain.com/users/car',
      uriUrl = 'http://domain.com/uris/car';
    ...
    it('streams user asset', (done) => {
      rest.get.withArgs(userUrl).returns(h([['a','b','c']]));
      fn(userUrl, 10)
        .collect().toCallback((err, data) => {
          assertItems(data, [
            {url: userUrl, data: ['a','b','c']}
          ]);
          done(err);
        });
    });

    it('streams uri asset', (done) => {
      rest.get.withArgs(uriUrl).returns(h([['a','b','c']]));
      fn(uriUrl, 10)
        .collect().toCallback((err, data) => {
          assertItems(data, [
            {url: uriUrl, data: ['a','b','c']}
          ]);
          done(err);
        });
    });
    ...
    it('emits error if there is an unrecognized resource type', (done) => {
      fn('domain.com/2019/01/some-slug', 10).collect().toCallback((err) => {
        expect(err.message).to.eql('Unable to GET domain.com/2019/01/some-slug: Must be a page, component, list, or user');
        done();
      });
    });
  });
  ...
  describe('streamUserUris', function () {
    const fn = lib[this.title];

    it ('returns a stream of list URIs from the /users endpoint', function () {
      const site = 'http://foo.com';

      rest.get.withArgs(`${site}/users`).returns(h([[1,2,3]]));
      return fn(site).collect().toPromise(Promise).then((results) => {
        expect(results).to.deep.eql([1,2,3]);
      });
    });

    it ('throws error if /users endpoint not found', function (done) {
      const site = 'http://foo.com';

      rest.get.withArgs(`${site}/users`).returns(h.fromError(createError(site)));
      fn(site).collect().toCallback((err) => {
        expect(err.message).to.equal('Not Found');
        done();
      });
  ...
  describe('streamUris', function () {
    const fn = lib[this.title];

    it ('returns a stream of list URIs from the /uris endpoint', function () {
      const site = 'http://foo.com';

      rest.get.withArgs(`${site}/uris`).returns(h([[1,2,3]]));
      return fn(site).collect().toPromise(Promise).then((results) => {
        expect(results).to.deep.eql([1,2,3]);
      });
    });

    it ('throws error if /uris endpoint not found', function (done) {
      const site = 'http://foo.com';

      rest.get.withArgs(`${site}/uris`).returns(h.fromError(createError(site)));
      fn(site).collect().toCallback((err) => {
        expect(err.message).to.equal('Not Found');
        done();
      });
  ...
  describe('streamPageAssets', function () {
    const fn = lib[this.title];

    it ('streams page assets', (done) => {
      const mockPageData = {
        foo: [
          'foo.com/components/bar/1',
          'foo.com/components/bar/2'
        ]
      };

      rest.get.returns(h([mockPageData]));
      fn('foo.com/pages/test').collect().toCallback((err, data) => {
        assertItems(data, [
          {url: 'foo.com/pages/test', data: mockPageData},
          {url: 'foo.com/components/bar/1'},
          {url: 'foo.com/components/bar/2'}
        ]);
        done(err);
      });
    });

    it ('streams layout cmpt and marks it as a layout', (done) => {
      const mockPageData = {
        layout: 'foo.com/components/bar/1'
      };
      ...
      fn('foo.com/pages/test').collect().toCallback((err, data) => {
        assertItems(data, [
          {url: 'foo.com/pages/test', data: mockPageData},
          {url: 'foo.com/components/bar/1', isLayout: true},
        ]);
        done(err);
      });
    });

    it ('ignores reserved page properties', (done) => {
      const mockPageData = {
        main: ['foo.com/components/bar/1'],
        url: 'foo',
        urlHistory: ['foo.com/foo/bar'],
        customUrl: 'foo.com/foo/bar',
        lastModified: 10309232,
        priority: 1,
        changeFrequency: 1
      };
    ...
    it ('ignores any string that isn\'t layout', (done) => {
      const mockPageData = {
        main: ['foo.com/components/bar/1'],
        something: 'foo.com/components/bar/2'
      };
