...
describe('input clay', () => {
  ...
  describe('getComponentInstances', () => {
    ...
    it('fetches component instances', (done) => {
      ...
      fn('domain.com', 'foo').collect().toCallback((err, data) => {
        ...
        expect(rest.get).to.have.been.calledWith('domain.com/_components/foo/instances');
        done(err);
      });
    });
    ...
    it('fetches published component instances', (done) => {
      ...
      fn('domain.com', 'foo', {concurrency: 1, headers: {headerKey: 'headerVal'}, onlyPublished: true}).collect().toCallback((err, data) => {
        expect(data).to.eql(['one']);
        expect(rest.get).to.have.been.calledWith('domain.com/_components/foo/instances/@published', {concurrency: 1, headers: {headerKey: 'headerVal'}});
        done(err);
      });
    });
  });
  ...
  describe('listComponentReferences', () => {
    ...
    it('does not return references from page data', () => {
      expect(fn({
        layout: 'domain.com/_components/foo',
        main: ['domain.com/_components/bar']
      })).to.eql([]);
    });
  });
  ...
  describe('checkAllReferences', () => {
    const fn = lib.checkAllReferences,
      fooUri = 'domain.com/_components/foo',
      fooUrl = `http://${fooUri}`,
      barUri = 'domain.com/_components/bar',
      barUrl = `http://${barUri}`,
      bazUri = 'domain.com/_components/baz',
      bazUrl = `http://${bazUri}`,
      pageUri = 'domain.com/_pages/foo',
      pageUrl = `http://${pageUri}`,
      publicUri = 'domain.com/some-slug',
      b64publicUri = b64.encode(publicUri),
      uriUrl = `http://domain.com/_uris/${b64publicUri}`;
  ...
  describe('importUrl', () => {
    ...
    it('imports single page', (done) => {
      ...
      rest.get.withArgs(fooJSON).returns(h([JSON.stringify({ a: 'b' })]));
      rest.get.withArgs(barJSON).returns(h([JSON.stringify({ c: 'd' })]));
      fn(pageUrl, 10).collect().toCallback((err, data) => {
        expect(data).to.eql([{ url: pageUrl, data: { layout: fooUri, a: [barUri] } }, { url: fooUrl, data: JSON.stringify({ a: 'b' }) }, { url: barUrl, data: JSON.stringify({ c: 'd' }) }]);
        done(err);
      });
    });
