...
function replace(str, prefix) {
  return new bluebird((resolve, reject) => {
    asyncReplace(str, /"_ref":"(.*?)\/components/g, `"_ref":"${prefix}/components`, (err, result) => {
...
function addPrefix(prefix, uri) {
  ...
  _.forEach(types, (type) => {
    if (_.includes(uri, type)) {
      if (type === '/uris') {
        let encoded = _.last(uri.split('/uris/'));

        prefixedUri = prefix + '/uris/' + b64.encode(prefix + encoded);
      } else {
...
/**
* Replace prefixes of a Clay URI asset (i.e. from the /uris endpoint) with the
* specified prefix.
* @param {string} prefix
* @param {Object} uriAsset of the form {url: string, data: Object}
* @return {Object} new uriAsset with prefixes replaced
**/
function replaceUriAssetPrefixes(prefix, uriAsset) {
  const aPrefix = urlUtil.getUrlPrefix(uriAsset.url).prefix,
    encodedA = uriAsset.url.split('/').pop(),
    decodedA = Buffer.from(encodedA, 'base64').toString(),
    decodedB = decodedA.replace(urlUtil.urlToUri(aPrefix), urlUtil.urlToUri(prefix)),
    encodedB = Buffer.from(decodedB).toString('base64');

  uriAsset.url = `${prefix}/uris/${encodedB}`;

  // Convert the URI data (which is a URI string, e.g. 'foo.com/pages/bar')
  // to a URL so we can replace the prefix, then back to a URI.
  uriAsset.data = urlUtil.urlToUri(urlUtil.uriToUrl(prefix, uriAsset.data));
...
function replacePrefixes(prefix) {
  ...
  return (item) => {
    if (_.isObject(item.data)) {
      ...
      return h.of(_.assign({}, item, {
        url: urlUtil.uriToUrl(prefix, item.url),
        // strip the prefixes, add the new prefixes, then stringify
        data: toggleReferencePrefixes(toggleReferencePrefixes(item.data, stripPrefix), addPrefix.bind(null, uriPrefix))
      }));
    } else if (_.isString(item.data)) {
      if (_.includes(item.url, '/uris/')) {
        h.of(replaceUriAssetPrefixes(prefix, item));
      }
