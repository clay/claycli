  ...
  describe('replacePrefixes', () => {
    ...
    it('replaces prefixes in object', (done) => {
      fn(prefixUrl)({ url: 'someurl/components/foo', data: { a: 'b' }}).collect().toCallback((err, data) => {
        expect(data).to.eql([{ url: prefixUrl + '/components/foo', data: { a: 'b' } }]);
        done(err);
      });
    });
    ...
    it('replaces prefixes in URI object', (done) => {
      const hostA = 'a.com',
        hostB = 'b.com',
        mockCanonicalUrl = `${hostA}/foo/bar`,
        encoded = Buffer.from(mockCanonicalUrl).toString('base64'),
        mockAsset = {
          url: `http://${hostA}/uris/${encoded}`,
          data: `${hostA}/pages/somepage`
        },
        expectedCanonicalUrl = `${hostB}/foo/bar`,
        expectedEncoded = Buffer.from(expectedCanonicalUrl).toString('base64'),
        expectedAsset = {
          url: `http://${hostB}/uris/${expectedEncoded}`,
          data: `${hostB}/pages/somepage`
        };

      fn(`http://${hostB}`)(mockAsset).collect().toCallback((err, data) => {
        expect(data).to.eql([expectedAsset]);
        done(err);
      });
    });
  });
