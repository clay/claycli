 ...
 */
function streamAssets(urls, concurrency = 1) {
  return createStream(urls)
    .flatMap(url => {
      if (clayUtils.isComponent(url)) {
        ...
        return rest.get(url, {concurrency}).map(data => ({url, data}));
      } else {
    ...
    .flatMap(item => fillMissingData(item, concurrency));
}
...
* Page cmpts will not have "data" initially; use composeAsset to fill.
* @param {string} url
* @param {number} [concurrency]
* @return {Stream}
**/
function streamPageAssets(url, concurrency = 1) {
...
**/
function fillMissingData(item, concurrency = 1) {
