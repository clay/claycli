 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkComponent(url, isRecursive) {
  const stream = rest.get(url)
    .flatMap((data) => {
      const childUris = listComponentReferences(data);
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkPage(url, isRecursive) {
  const stream = rest.get(url)
    .flatMap((data) => {
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkPublicUrl(url, prefix, isRecursive) {
  let publicUrl;
  ...
  return rest.get(publicUrl, null, 'text')
    .flatMap((pageUri) => {
      const pageUrl = config.normalizeSite(pageUri);

      return checkPage(pageUrl, isRecursive);
    }).errors(pushRestError);
}
 ...
 * @return {Stream} of { result: 'success or error', url (if error) }
 */
function checkAllReferences(uri, isRecursive, prefix) {
  const url = config.normalizeSite(uri);

  if (clayUtils.isComponent(url)) {
    return checkComponent(url, isRecursive);
  } else if (clayUtils.isPage(url)) {
    return checkPage(url, isRecursive);
  } else {
    return checkPublicUrl(url, prefix, isRecursive);
  }
}
