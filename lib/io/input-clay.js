 ...
 * @param  {string} url
 * @param  {number} [concurrency]
 * @return {Stream} of {url: string, data: object, isLayout: boolean, isPage: boolean} objects
 */
function getDeepPage(url, concurrency) {
  ...
  return rest.get(url, {concurrency})
    .flatMap((pageData) => {
      ...
      return h([
        h([layoutUrl])
          .flatMap(url => getDeepComponent(url, concurrency))
          .doto(item => item.isLayout = true),
        h.of({ url, data: pageData}), // page data sent as object
        h(childUrls).flatMap(url => getDeepComponent(url, concurrency))
      ])
      .merge();
    });
}
...
function importUrl(url, concurrency) {
  if (clayUtils.isComponent(url)) {
    ...
    return getDeepPage(url, concurrency);
  } else {
