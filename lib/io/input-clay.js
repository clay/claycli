 ...
 * @param  {string} url
 * @param  {number} [concurrency]
 * @param  {boolean} [includeLayout] Include layouts in stream
 * @return {Stream}
 */
function getDeepPage(url, concurrency, includeLayout = false) {
  const prefix = urlUtil.getUrlPrefix(url).prefix;

  return rest.get(url, {concurrency})
    .flatMap((pageData) => {
      let childUrls = includeLayout ? [pageData.layout] : [];
 ...
 * @param  {string} url
 * @param  {number} concurrency
 * @param  {boolean} includeLayouts If true, include page layout instances in stream
 * @return {Stream} of objects with { url, data: stringified json }
 */
function importUrl(url, concurrency, includeLayouts) {
  if (clayUtils.isComponent(url)) {
    return getDeepComponent(url, concurrency);
  } else if (url.includes('/lists/')) {
    return rest.get(url, concurrency);
  } else if (clayUtils.isPage(url)) {
    return getDeepPage(url, concurrency, includeLayouts);
  } else {
