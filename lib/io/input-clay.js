...
function pushRestError(err, push) {
  push(null, { status: 'error', url: err.url, error: err }); // every url that errors out should be captured
}
...
function importUrl(url, concurrency) {
  if (clayUtils.isComponent(url)) {
    ...
    return streamPageItems(url, concurrency)
      .flatMap(item => composeItem(item, concurrency));
  } else {
...
/**
* GET the page at the specified URL, and return a stream of page items of
* the form {url: string, data: object}, including the page object itself.
* Page cmpts will not have data initially; use composeItem to fill with data.
* @param {string} url
* @param {number} concurrency
* @return {Stream}
**/
function streamPageItems(url, concurrency) {
  const prefix = urlUtil.getUrlPrefix(url).prefix;

  return rest.get(url, {concurrency})
    .flatMap((pageData) => {
      const layoutUri = pageData.layout,
        layoutUrl = urlUtil.uriToUrl(prefix, layoutUri);
      let childUrls;

      // remove reserved properties that aren't URIs or URI arrays
      pageData = _.omit(pageData, ['url', 'urlHistory', 'customUrl', 'lastModified', 'priority', 'changeFrequency', 'layout']);
      childUrls = _.reduce(pageData, (uris, area) => {
        return _.isArray(area) ? uris.concat(area) : uris;
      }, [])
      .map(urlUtil.uriToUrl.bind(null, prefix));

      return h([
        h([layoutUrl]).map(url => ({url, isLayout: true})),
        h(childUrls).map(url => ({url})),
        h([{ url, data: pageData}]) // page data sent as object
      ])
      .merge();
    });
}

function composeItem(item, concurrency) {
  if (item.data) return h.of(item); // item already has data
  return rest.get(item.url + '.json', {concurrency})
    .map(data => ({url: item.url, data}));
}

module.exports.streamPageItems = streamPageItems;
module.exports.composeItem = composeItem;
...
module.exports.getListsInSite = getListsInSite;
module.exports.getPagesInSite = getPagesInSite;
