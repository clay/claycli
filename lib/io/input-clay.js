...
/**
 * get a component and its children
 * @param  {number} concurrency
 * @param  {string} uri or url
 * @return {Stream}
 */
function recursivelyGetComponent(concurrency, uri) {
  const url = config.normalizeSite(uri);
  ...
  return rest.get(url, concurrency)
    .flatMap((data) => {
      ...
      return h([
        h.of(chunks.toChunk(url, data)),
        h(childUris)
          .flatMap(recursivelyGetComponent.bind(null, concurrency))
      ]).mergeWithLimit(concurrency);
    }).errors(pushRestError);
}

/**
 * recursively get a component or page, returning a stream of chunks
 * @param  {string} url
 * @param  {number} concurrency
 * @return {Stream}
 */
function recursiveGet(url, concurrency) {
  if (clayUtils.isComponent(url)) {
    return recursivelyGetComponent(concurrency, url);
  } else if (clayUtils.isPage(url)) {
    return rest.get(url, concurrency)
      .flatMap((pageData) => {
        const layoutUri = data.layout,
        ...
        return h([
          h.of(chunks.toChunk(url, pageData)), // chunk page data
          h([layoutUri].concat(otherComponentUris)) // send layout and other components
            .flatMap(recursivelyGetComponent.bind(null, concurrency)) // to get their data
        ]).mergeWithLimit(concurrency);
      }).errors(pushRestError);
  } else {
...
module.exports.recursiveGet = recursiveGet;
