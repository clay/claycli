...
describe('rest', () => {
  ...
  describe('get', () => {
    ...
    it('gets text', () => {
      ...
      return lib.get(url, { type: 'text' }).then((res) => {
        expect(res).toBe('hi');
      });
    });
    ...
    it('uses headers', () => {
      fetch.mockResponseOnce(JSON.stringify({ a: 'b' }));
      return lib.get(url, { headers: { Authorization: 'Token abc' }}).then(() => {
  ...
  describe('put', () => {
    ...
    it('sends json', () => {
      ...
      return lib.put(url, json, { key }).then((res) => {
    ...
    it('sends json with empty body', () => {
      fetch.mockResponseOnce(json);
      return lib.put(url, null, { key }).then((res) => {
        expect(res).toEqual({ type: 'success', message: url });
        expect(fetch).toHaveBeenCalledWith(url, {
          method: 'PUT',
          agent: null,
    ...
    it('sends text', () => {
      fetch.mockResponseOnce('hi');
      return lib.put(url, 'hi', { type: 'text', key }).then((res) => {
        expect(res).toEqual({ type: 'success', message: url });
        expect(fetch).toHaveBeenCalledWith(url, {
          method: 'PUT',
          agent: null,
    ...
    it('uses https agent for ssl calls', () => {
      fetch.mockResponseOnce(json);
      return lib.put(url2, json, { key }).then(() => {
        expect(fetch.mock.calls[0][1].agent).not.toBeNull();
      });
    });
    ...
    it('uses headers', () => {
      fetch.mockResponseOnce(json);
      return lib.put(url, json, { key, headers: { some_header: 'value' }}).then(() => {
    ...
    it('captures url in error details on network rejection', () => {
      ...
      return lib.put(url, { a: 'b' }, { key }).then((res) => {
  ...
  describe('query', () => {
    ...
    it('returns error if elastic errors', () => {
      ...
      return lib.query(url, json, { key }).then((res) => {
    ...
    it('uses https agent for ssl calls', () => {
      fetch.mockResponseOnce(resultsString);
      return lib.query(url2, json, { key }).then(() => {
        expect(fetch.mock.calls[0][1].agent).not.toBeNull();
      });
    });
    ...
    it('uses headers', () => {
      fetch.mockResponseOnce(resultsString);
      return lib.query(url, json, { key, headers: { some_header: 'value' }}).then(() => {
        expect(fetch.mock.calls[0][1].headers).toEqual({
          Authorization: `Token ${key}`,
          'Content-Type': 'application/json; charset=UTF-8',
          some_header: 'value'
  ...
  describe('findURI', () => {
    it('finds page uri with one hop', () => {
      ...
      return lib.findURI('http://domain.com/some-slug.html').then((res) => {
    ...
    it('works with ssl', () => {
      fetch.mockResponseOnce('domain.com/_pages/foo');
      return lib.findURI('https://domain.com/some-slug.html').then((res) => {
    ...
    it('finds page uri with two hops', () => {
      ...
      fetch.mockResponseOnce('domain.com/_pages/foo');
      return lib.findURI('http://domain.com/path/some-slug.html').then((res) => {
        expect(res).toEqual({ uri: 'domain.com/_pages/foo', prefix: 'http://domain.com' });
      });
    });
    ...
    it('finds page uri with three hops (deep path)', () => {
      ...
      return lib.findURI('http://domain.com/a/b/some-slug.html').then((res) => {
        expect(res).toEqual({ uri: 'domain.com/_pages/foo', prefix: 'http://domain.com' });
      });
    });
    ...
    it('encodes public URI as base64 in _uris lookup', () => {
      fetch.mockResponseOnce('domain.com/_pages/foo');
      return lib.findURI('http://domain.com/some-slug.html').then(() => {
    ...
    it('rejects if no relevant api route', () => {
      ...
      return lib.findURI('http://domain.com/some-slug.html').catch((e) => {
  ...
  describe('isElasticPrefix', () => {
    it('returns true if _components endpoint exists at prefix', () => {
      ...
      return lib.isElasticPrefix('http://domain.com').then((res) => {
    ...
    it('works for ssl', () => {
      fetch.mockResponseOnce('{}');
      return lib.isElasticPrefix('https://domain.com').then(() => {
        expect(fetch.mock.calls[0][1].agent).not.toBeNull();
      });
    });
