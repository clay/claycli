...
describe('rest', () => {
  ...
  it('captures url on error object from get()', () => {
    fetch.mockRejectOnce(new Error('ECONNREFUSED'));
    return lib.get(url).toPromise(Promise).then((res) => {
      // error is returned as an Error object with .url set
  ...
  describe('findURI', () => {
    ...
    it('finds page uri with three hops (deep path)', () => {
      ...
      return lib.findURI('http://domain.com/a/b/some-slug.html').toPromise(Promise).then((res) => {
        expect(res).toEqual({ uri: 'domain.com/_pages/foo', prefix: 'http://domain.com' });
      });
    });
    ...
    it('encodes public URI as base64 in _uris lookup', () => {
      fetch.mockResponseOnce('domain.com/_pages/foo');
      return lib.findURI('http://domain.com/some-slug.html').toPromise(Promise).then(() => {
  ...
  describe('get with no agent for http', () => {
    it('passes null agent for non-ssl urls', () => {
      fetch.mockResponseOnce(JSON.stringify({ a: 'b' }));
      return lib.get(url).toPromise(Promise).then(() => {
        expect(fetch.mock.calls[0][1].agent).toBeNull();
      });
    });
  });

  describe('put error details', () => {
    var key = 'some api key';

    it('captures url in error details on network rejection', () => {
      fetch.mockRejectOnce(new Error('ECONNREFUSED'));
      return lib.put(url, { a: 'b' }, { key }).toPromise(Promise).then((res) => {
        expect(res).toEqual({ type: 'error', details: url, message: 'ECONNREFUSED' });
      });
    });
  });
