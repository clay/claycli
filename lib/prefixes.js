...
function add(dispatch, prefix) {
  ...
  }).then((prefixedString) => replace(prefixedString, /"(customUrl|url)":"([^"]*)"/g, (match, prop, uri) => {
    if (uri.startsWith('http')) {
      // URI already has a host (e.g. importing without prior export strip); replace origin with target origin
      const targetOrigin = urlPrefix.match(/^(https?:\/\/[^/]+)/)[1],
        path = uri.replace(/^https?:\/\/[^/]+/, '');

      return Promise.resolve(`"${prop}":"${targetOrigin}${path}"`);
    }
    return Promise.resolve(`"${prop}":"${urlPrefix}${uri}"`);
  }))).map(JSON.parse);
}
...
function remove(dispatch, prefix) {
  ...
  }).then((unprefixedString) => replace(unprefixedString, /"(customUrl|url)":"([^"]*)"/g, (match, prop, prefixedURI) => {
    const path = prefixedURI.startsWith('http') ? prefixedURI.replace(/^https?:\/\/[^/]+/, '') : prefixedURI.replace(urlPrefix, '');

    return Promise.resolve(`"${prop}":"${path}"`);
  }))).map(JSON.parse);
}
