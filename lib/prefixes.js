...
function add(dispatch, prefix) {
  ...
  return replace(stringDispatch, /"\/_?(components|uris|pages|lists|users|layouts)(\/[\w-\/]*)/g, (match, type, name) => {
    if (type === 'uris') {
      return Promise.resolve(`"${prefix}/_${type}/${b64.encode(prefix + name)}`);
    } else {
      return Promise.resolve(`"${prefix}/_${type}${name}`);
    }
  }).then((prefixedString) => replace(prefixedString, /"customUrl":"(.*)"/g, (match, uri) => Promise.resolve(`"customUrl":"${urlPrefix}${uri}"`))).then(JSON.parse);
}
...
function remove(dispatch, prefix) {
  ...
  return replace(stringDispatch, new RegExp(`"${prefix}\/_?(components|uris|pages|lists|users|layouts)/(.+?)"`, 'g'), (match, type, end) => {
    if (type === 'uris') {
      return Promise.resolve(`"/_${type}${b64.decode(end).replace(prefix, '')}"`);
    } else {
      return Promise.resolve(`"/_${type}/${end}"`);
    }
  }).then((unprefixedString) => replace(unprefixedString, /"customUrl":"(.*)"/g, (match, prefixedURI) => Promise.resolve(`"customUrl":"${prefixedURI.replace(urlPrefix, '')}"`))).then(JSON.parse);
}
