...
/**
 * get the path we output component css to
 * @param  {string} component name
 * @param  {string} site      slug
 * @param  {string} basePath  cwd of your clay install
 * @return {string}
 */
function getOutputPath(component, site, basePath) {
  return `${basePath}/${defaultAssetDir}/css/${component}.${site}.css`;
}

/**
 * gracefully catch and display postcss errors
 * @param  {error} e
 */
function catchPostcssError(e) {
  ...
  return fs.readFile(componentPath, 'utf8')
    .then((contents) => {
      ...
      return postcss(plugins)
        .process(contents, {
          from: componentPath,
          to: getOutputPath(component, site, basePath),
          parser: syntax
        })
        .then((result) => fs.outputFile(getOutputPath(component, site, basePath), result.css))
        .catch(catchPostcssError);
    });
}
