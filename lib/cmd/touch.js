...
function runLogic(prefix, name, argv) {
  const processSpinner = logger.startSpinner('Processing...'),
    instancesStream = clay.getComponentInstances(prefix, name).stopOnError((err) => {
  ...
  if (argv.n) {
    ...
    instancesStream.flatMap((instances) => {
      ...
      return rest.get(urls).stopOnError((err) => {
        logger.stopSpinner(processSpinner);
