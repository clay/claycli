...
describe('import', () => {
  ...
  it('returns error if not JSON', () => {
    return lib('abc', url).toPromise(Promise).then((res) => {
      expect(res).toEqual({ type: 'error', message: 'JSON syntax error: Invalid JSON' });
    });
  });

  it('returns error if bad JSON', () => {
    return lib('{a}', url).toPromise(Promise).then((res) => {
      expect(res).toEqual({ type: 'error', message: 'JSON syntax error: Unexpected token a in JSON at position 1' });
    });
  });
