...
describe('import', () => {
  ...
  it('returns error if bad JSON', () => {
    return lib(']{"a"\:}', url).toPromise(Promise).then((res) => {
      ...
      expect(res.message).toContain('JSON syntax error:');
  ...
  it('handles empty YAML input gracefully', () => {
    return lib('', url, { yaml: true, key, concurrency }).collect().toPromise(Promise).then((res) => {
      expect(res).toEqual([]);
    });
  });

  it('handles empty JSON dispatch input gracefully', () => {
    return lib('', url, { key, concurrency }).collect().toPromise(Promise).then((res) => {
      expect(res).toEqual([]);
    });
  });

  it('returns error for deeply invalid JSON that is also not YAML', () => {
    return lib('}{][', url, { key, concurrency }).toPromise(Promise).then((res) => {
      expect(res.type).toBe('error');
      expect(res.message).toContain('JSON syntax error');
    });
  });
