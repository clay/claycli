...
describe('lint', () => {
  ...
  describe('lintUrl', () => {
    ...
    it('lints with default concurrency', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar').then((res) => {
    ...
    it('lints an existing layout without children', () => {
      fetch.mockResponseOnce('{}');
      return lib.lintUrl('domain.com/_layouts/foo/instances/bar', {concurrency}).then((res) => {
    ...
    it('lints an existing component without children', () => {
      fetch.mockResponseOnce('{}');
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).then((res) => {
        expect(res).toEqual([{ type: 'success', message: 'http://domain.com/_components/foo/instances/bar' }]);
      });
    });
    ...
    it('lints an existing component with extension, without children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar.html', {concurrency}).then((res) => {
    ...
    it('lints an existing page with existing children', () => {
      ...
      return lib.lintUrl('domain.com/_pages/foo', {concurrency}).then((res) => {
    ...
    it('lints an existing page with existing children with broken template', () => {
      ...
      return lib.lintUrl('domain.com/_pages/foo.html', {concurrency}).then((res) => {
    ...
    it('lints an existing public url with existing children', () => {
      ...
      return lib.lintUrl('domain.com/some-slug', {concurrency}).then((res) => {
  ...
  describe('lintUrl deep nesting', () => {
    ...
    it('returns error for unreachable public url', () => {
      ...
      return lib.lintUrl('domain.com/nonexistent-slug', {concurrency}).then((res) => {
  ...
  describe('lintSchema', () => {
    it('returns error if yaml syntax error', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi' }) + 'a').then((res) => {
        expect(res[0]).toMatchObject({ type: 'error', message: expect.stringMatching(/^YAML syntax error/) });
      });
    });

    it('returns error if no _description', () => {
      return lib.lintSchema(yaml.dump({ foo: { _has: 'bar' } })).then((res) => {
        expect(res).toEqual([{ type: 'error', message: 'Schema has no _description' }]);
      });
    });

    it('lints _description', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi' })).then((res) => {
        expect(res).toEqual([{ type: 'success' }]);
      });
    });

    it('returns error if non-camelCased prop', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi', 'foo-bar': { _has: 'baz' } })).then((res) => {
        expect(res).toEqual([{ type: 'error', message: 'Properties must be camelCased', details: 'foo-bar' }]);
      });
    });

    it('lints camelCased prop', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi', fooBar: { _has: 'baz' } })).then((res) => {
        expect(res).toEqual([{ type: 'success' }]);
      });
    });

    it('returns error if non-existant group fields', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi', _groups: { foo: { fields: ['bar'] } } })).then((res) => {
    ...
    it('lints existing group fields', () => {
      return lib.lintSchema(yaml.dump({ _description: 'hi', bar: { _has: 'baz' }, _groups: { foo: { fields: ['bar'] } } })).then((res) => {
        expect(res).toEqual([{ type: 'success' }]);
      });
    });

    it('returns multiple errors for missing description and non-camelCase props', () => {
      return lib.lintSchema(yaml.dump({ 'foo-bar': { _has: 'baz' } })).then((res) => {
    ...
    it('lints schema with only _description and no fields', () => {
      return lib.lintSchema(yaml.dump({ _description: 'A simple component' })).then((res) => {
        expect(res).toEqual([{ type: 'success' }]);
      });
    });
  });
