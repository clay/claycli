  ...
  describe('lintUrl', () => {
    it('lints with default concurrency', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar').toPromise(Promise).then((res) => {
        expect(res).toEqual({ action: 'success', message: 'http://domain.com/_components/foo/instances/bar' });
      });
    });
    ...
    it('lints a non-existing component without children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'error', message: 'http://domain.com/_components/foo/instances/bar' }]);
      });
    });
    ...
    it('lints a component with existing children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_components/foo/instances/bar' }, { action: 'success', message: 'http://domain.com/_components/some-child' }]);
      });
    });
    ...
    it('lints a component with non-existing children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_components/foo/instances/bar' }, { action: 'error', message: 'http://domain.com/_components/some-child' }]);
      });
    });
    ...
    it('lints a component with existing deep children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_components/foo/instances/bar' }, { action: 'success', message: 'http://domain.com/_components/bar' }, { action: 'success', message: 'http://domain.com/_components/baz' }]);
      });
    });
    ...
    it('lints a component with non-existing deep children', () => {
      ...
      return lib.lintUrl('domain.com/_components/foo/instances/bar', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_components/foo/instances/bar' }, { action: 'success', message: 'http://domain.com/_components/bar' }, { action: 'error', message: 'http://domain.com/_components/baz' }]);
      });
    });
    ...
    it('lints an existing page with existing children', () => {
      ...
      return lib.lintUrl('domain.com/_pages/foo', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_pages/foo' }]);
      });
    });
    ...
    it('lints an existing page with non-existing children', () => {
      ...
      return lib.lintUrl('domain.com/_pages/foo', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/_pages/foo' }, { action: 'success', message: 'http://domain.com/_components/foo/instances/bar' }, { action: 'error', message: 'http://domain.com/_components/bar/instances/baz' }]);
      });
    });
    ...
    it('lints an existing public url with existing children', () => {
      ...
      return lib.lintUrl('domain.com/some-slug', {concurrency}).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'success', message: 'http://domain.com/some-slug' }, { action: 'success', message: 'http://domain.com/_pages/foo' }]);
      });
    });
  });
  ...
  describe('lintSchema', () => {
    it('returns error if yaml syntax error', () => {
      return lib.lintSchema(yaml.safeDump({ _description: 'hi' }) + 'a').toPromise(Promise).then((res) => {
        expect(res).toEqual({ action: 'error', message: 'YAML syntax error: can not read a block mapping entry; a multiline key may not be an implicit key at line 3, column 1' });
      });
    });

    it('returns error if no _description', () => {
      return lib.lintSchema(yaml.safeDump({ foo: { _has: 'bar' } })).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'error', message: 'Schema has no _description' }]);
      });
    });

    it('lints _description', () => {
      return lib.lintSchema(yaml.safeDump({ _description: 'hi' })).toPromise(Promise).then((res) => {
        expect(res).toEqual({ action: 'success' });
      });
    });
    ...
    it('returns error if non-camelCased prop', () => {
      return lib.lintSchema(yaml.safeDump({ _description: 'hi', 'foo-bar': { _has: 'baz' } })).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'error', message: 'Properties must be camelCased', details: 'foo-bar' }]);
      });
    });
    ...
    it('returns error if non-existant group fields', () => {
      return lib.lintSchema(yaml.safeDump({ _description: 'hi', _groups: { foo: { fields: ['bar'] } } })).collect().toPromise(Promise).then((res) => {
        expect(res).toEqual([{ action: 'error', message: 'Fields referenced by groups don\'t exist', details: 'foo » bar' }]);
      });
    });
