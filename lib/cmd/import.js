...
function showCompleted(results) {
  ...
  if (skippedResults.length) {
    ...
    logger.info('Use --overwrite to specify the resource types to overwrite (use "all", "none", or a combination of "components", "layouts", "pages", "lists", or "users")');
  }
...
function importUrl(argv) {
  ...
  const url = argv.page || argv.component || argv.user || argv.list,
    prefix = argv.prefix,
...
function normalizeArgv(argv) {
  ...
  if (argv.overwrite && argv.overwrite.length === 1) {
    argv.overwrite = argv.overwrite[0];
  }
}
...
function handler(argv) {
  ...
  } else if (argv.component || argv.page || argv.list || argv.user) {
    importUrl(argv);
  } else if (argv.file) {
    importFile(argv);
  } else if (argv.site) {
