...
function showProgress(argv) {
  ...
  return (result) => {
    ...
    if (argv.verbose && result.status === 'success') {
      ...
      logger.debug(`${chalk.red('✗')} ${result.err.method} ${result.err.url}`);
    ...
    if (!argv.verbose) {
      singleLineLog(`${'-'.repeat(45)}\nImported URIs: ${chalk.green(successCount)} \t errors: ${chalk.red(errorCount)}`);
    }
    return result;
  };
}
...
function handler(argv) {
  ...
  if (_.isUndefined(process.stdin.isTTY)) {
    ...
    importUrl(config.normalizeSite(argv.component), prefix, argv);
  } else if (argv.page) {
    importUrl(config.normalizeSite(argv.page), prefix, argv);
  } else if (argv.site) {
    importSite(config.getSite(argv.site), prefix, argv);
  } else if (argv.file) {
    importFile.importFile(config.getFile(argv.file), prefix, argv);
  } else {
