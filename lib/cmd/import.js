...
async function importBootstrap(obj, prefix, key, options) {
  var dispatches = formatting.toDispatch([obj]),
    results = [], i, prefixed, dispatchResults;
...
async function importJson(source, prefix, key, options) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (_.isString(obj)) {
      try {
        ...
        try {
          ...
          results.push({ type: 'error', message: `JSON syntax error: ${e.message}`, details: _.truncate(obj) });
          continue;
        }
      }
    }
    if (obj.type && obj.type === 'error') {
      ...
      dispatchResults = await importDispatch(obj, prefix, key, options);
      results = results.concat(dispatchResults);
    }
  }
  return results;
