...
function importSite(site, prefix, argv) {
  const pagesEndpoint = site + '/pages',
    {limit, offset} = argv,
    {protocol} = nodeUrl.parse(site);

  if (argv.users) {
    console.error('the users option is not yet implemented');
    process.exit(1);
  }
  if (argv.lists) {
  ...
  return rest.get(pagesEndpoint)
    .map(pages => pages
      .slice(pages.length - offset - limit, pages.length - offset)
      .reverse())
    .flatten()
    .flatMap(url => importSingleUrl(`${protocol}//${url}`, prefix, argv));
}
...
function handler(argv) {
  ...
  if (_.isUndefined(process.stdin.isTTY)) {
    fnc = importStream(prefix, argv);
  } else if (argv.component) {
    fnc = importSingleUrl(config.normalizeSite(argv.component), prefix, argv);
  } else if (argv.page) {
    fnc = importSingleUrl(config.normalizeSite(argv.page), prefix, argv);
  } else if (argv.site) {
    fnc = importSite(config.getSite(argv.site), prefix, argv);
  } else if (argv.file) {
    fnc = importFile(config.getFile(argv.file), prefix, argv);
  } else {
    logger.error('Please specify somewhere to import from!');
    process.exit(1);
  }

  fnc.map(showProgress(argv)).toArray(showCompleted);
}
