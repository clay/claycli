...
function importItems(str, url, options = {}) {
  ...
  if (options.yaml) {
    // parse bootstraps
    return (h.isStream(str) ? str : h.of(str))
    ...
    return (h.isStream(str) ? str : h.of(str))
      .flatMap((res) => {
        // if it's a stream of strings, split them on newlines and discard any empty lines
        return _.isString(res) ? h.of(res).split().compact() : h.of(res);
      })
      .map((res) => {
        try {
