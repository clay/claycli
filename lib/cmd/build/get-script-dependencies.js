...
/**
 * Returns true when the chunk should be included in the output scripts list.
 * Skips already-seen chunks, and when chunksOnly is true also skips individual
 * component client files (only shared /chunks/ paths pass through).
 *
 * @param {string}  chunk
 * @param {boolean} chunksOnly
 * @param {Set}     seen
 * @returns {boolean}
 */
function shouldIncludeChunk(chunk, chunksOnly, seen) {
  if (seen.has(chunk)) return false;
  if (chunksOnly && !chunk.includes('/chunks/')) return false;
  return true;
}
 ...
 * @param {object}  [opts]
 * @param {string}  [opts.base]       - Public URL base prefix e.g. '/js'
 * @param {boolean} [opts.chunksOnly] - Skip non-shared-chunk imports
 * @returns {string[]}
 */
function collectScripts(key, manifest, seen, opts) {
  const { base = '/js', chunksOnly = false } = opts || {};
  ...
  if (!seen.has(entry.file)) {
    seen.add(entry.file);
    scripts.push(entry.file.replace(/^\/js/, base));
  }

  for (const chunk of entry.imports || []) {
    if (!shouldIncludeChunk(chunk, chunksOnly, seen)) continue;
    seen.add(chunk);
    scripts.push(chunk.replace(/^\/js/, base));
  }
...
function getDependenciesNextForComponents(assetPath, globalKeys) {
  ...
  if (manifest[VIEW_INIT_ENTRY_KEY]) {
    result.push(...collectScripts(VIEW_INIT_ENTRY_KEY, manifest, seen, { base, chunksOnly: true }));
  }

  // Global entry points (aaa-module-mounting, ads, facebook, cid, …)
  for (const key of (globalKeys || [])) {
    if (manifest[key]) {
      result.push(...collectScripts(key, manifest, seen, { base, chunksOnly: true }));
    }
  }
...
function getEditDeps(manifest, base, seen) {
  ...
  for (const key of Object.keys(manifest)) {
    if (key.endsWith('/model') || key.endsWith('/kiln')) {
      result.push(...collectScripts(key, manifest, seen, opts));
    }
  }
  ...
  if (manifest[kilnKey]) {
    result.push(...collectScripts(kilnKey, manifest, seen, opts));
  }
...
/**
 * Return the set of hashed script URLs that should be emitted as
 * `<link rel="modulepreload">` hints in the page `<head>`.
 *
 * Identical to `getDependenciesNextForComponents` — the URLs are the same;
 * the difference is purely how the caller uses them (preload hints vs script
 * tags).  Keeping them as a separate function makes the intent explicit in
 * `resolveMedia` and allows the two lists to diverge in the future if needed
 * (e.g. only preloading a subset of the globals).
 *
 * Returns an empty array when no manifest is present so that sites still
 * running the legacy Browserify pipeline are unaffected.
 *
 * @param {string}   assetPath  - Site asset prefix (e.g. 'https://cdn.example.com')
 * @param {string[]} [globalKeys] - Same keys passed to getDependenciesNextForComponents
 * @returns {string[]}
 */
function getModulePreloadHints(assetPath, globalKeys) {
  if (!hasManifest()) return [];
  return getDependenciesNextForComponents(assetPath, globalKeys);
}

module.exports = { getDependenciesNext, getDependenciesNextForComponents, getModulePreloadHints, hasManifest, getTemplatePaths, getEditScripts };
