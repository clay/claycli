...
async function writeManifest(metafile, outdir, publicBase = '/js') {
  ...
  for (const [outputAbsPath, outputInfo] of Object.entries(metafile.outputs)) {
    ...
    // Only include static import-statements — dynamic-import() calls are
    // resolved at runtime by the bundle itself and must not become eager
    // <script> tags (doing so generated 500+ requests for _kiln-edit-init).
