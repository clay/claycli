...
// global/js/*.js is intentionally excluded here — it is built as a single
// non-splitting bundle (_globals-init) via generateGlobalsInitEntry() so that
// the browser loads one file instead of the 70–100 tiny shared chunks that
// esbuild's splitter would produce from the overlapping global scripts.
//
// model.js files are intentionally excluded from this splitting pass. They are
// server-side Amphora lifecycle hooks (loaded by Node's require() at runtime,
// never fetched by the browser) and are also bundled into _kiln-edit-init via
// generateKilnEditEntry(). Including them here would add ~322 entry points to
// the splitting graph, creating artificial crossing edges between model and
// client dependency graphs and inflating the shared chunk count by hundreds of
// files. The _kiln-edit-init bundle (splitting: false) already provides
// clay-kiln with all component models via window.kiln.componentModels.
const ENTRY_GLOBS = [
...
// Generated globals bundle: all global/js/*.js files concatenated into a
// single non-splitting bundle so the browser loads one file instead of the
// 70–100 shared chunks esbuild would otherwise produce from them.
const GLOBALS_INIT_ENTRY_FILE = path.join(CLAY_DIR, '_globals-init.js');
const GLOBALS_INIT_ENTRY_KEY  = '.clay/_globals-init';
...
async function generateKilnEditEntry() {
  ...
  // Browserify populated window.modules at runtime; the esbuild pipeline does
  // not. Shim it so clay-kiln's preloader doesn't crash on Object.keys(window.modules)
  // when running alongside the new pipeline. The object will be empty (correct —
  // there are no Browserify modules to enumerate) and the real registries are
  // pre-populated above via window.kiln.componentModels / componentKilnjs.
...
/**
 * Generate the globals bundle entry point.
 *
 * Creates `.clay/_globals-init.js` which imports all `global/js/*.js` files
 * (excluding test files) into a single entry point.  This file is then built
 * with `splitting: false` so the browser receives one file instead of the
 * 70–100 tiny shared chunks that esbuild's splitter would otherwise produce
 * from the overlapping `global/js` modules.
 *
 * Sites that previously referenced individual global keys
 * (`global/js/aaa-module-mounting`, `global/js/ads`, …) in their
 * `GLOBAL_KEYS` array should switch to the single `'.clay/_globals-init'` key.
 *
 * @returns {Promise<string|null>} absolute path to the generated file, or null
 *   if no global/js/*.js files exist in this project
 */
async function generateGlobalsInitEntry() {
  ...
  const lines = [
    '// AUTO-GENERATED by clay build — DO NOT EDIT.',
    `// Generated: ${new Date().toISOString()}`,
    '// Bundles all global/js/*.js scripts into a single non-splitting bundle.',
    '// Built with splitting:false so the browser loads one file instead of',
    '// the many tiny shared chunks esbuild would otherwise produce.',
    '',
  ];
  ...
  await fs.ensureDir(CLAY_DIR);
  await fs.writeFile(GLOBALS_INIT_ENTRY_FILE, lines.join('\n'), 'utf8');

  return GLOBALS_INIT_ENTRY_FILE;
}
...
async function buildJS(options = {}) {
  ...
  // Build the main entry points (components, layouts, _view-init)
  ...
  // Build _globals-init as a separate self-contained bundle with splitting
  // disabled. The global/js/*.js files share a lot of code; letting esbuild
  // split them produces 70–100 tiny chunks that all load on every page.
  // Bundling them together gives the browser a single file per page load.
  const globalsResult = fs.existsSync(GLOBALS_INIT_ENTRY_FILE)
    ? await esbuild.build({ ...noSplitConfig, entryPoints: [GLOBALS_INIT_ENTRY_FILE] })
    : null;

  // Merge all metafiles so the manifest contains entries for all outputs.
  const mergedMetafile = {
    inputs: {
      ...result.metafile.inputs,
      ...kilnResult.metafile.inputs,
      ...globalsResult ? globalsResult.metafile.inputs : {},
    },
    outputs: {
      ...result.metafile.outputs,
      ...kilnResult.metafile.outputs,
      ...globalsResult ? globalsResult.metafile.outputs : {},
    },
  };
...
async function watch(options = {}) {
  ...
  const esbuildReady = (async () => {
    ...
    // Build _globals-init once upfront as a self-contained non-splitting
    // bundle so the browser loads one file for all global scripts instead of
    // the 70–100 tiny shared chunks esbuild's splitter would produce.
    const globalsResult = fs.existsSync(GLOBALS_INIT_ENTRY_FILE)
      ? await esbuild.build({ ...noSplitConfig, entryPoints: [GLOBALS_INIT_ENTRY_FILE] })
      : null;

    const kilnMetaOutputs = kilnResult.metafile.outputs;
    const globalsMetaOutputs = globalsResult ? globalsResult.metafile.outputs : {};
...
module.exports = { build, buildAll, buildJS, watch, getEsbuildConfig, generateKilnEditEntry, KILN_EDIT_ENTRY_KEY, generateViewInitEntry, VIEW_INIT_ENTRY_KEY, generateGlobalsInitEntry, GLOBALS_INIT_ENTRY_KEY, generateClientEnv };
