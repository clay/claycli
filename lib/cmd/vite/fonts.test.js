...
afterEach(async () => {
  ...
  delete process.env.CLAYCLI_COMPILE_LINKED_FONTS;
...
describe('buildFonts', () => {
  ...
  it('does not self-host .css files when linked fonts are disabled (default)', async () => {
    const { buildFonts } = require('./fonts');
    ...
    expect(fs.existsSync(mobileOut)).toBe(true);
    // With linked fonts off, a css-only styleguide creates no public/fonts dir
    expect(fs.existsSync(path.join(tmpDir, 'public', 'fonts', 'mobile'))).toBe(false);
  });

  it('self-hosts .css declaration files to public/fonts/{sg}/ when linked fonts are enabled', async () => {
    process.env.CLAYCLI_COMPILE_LINKED_FONTS = 'true';

    const { buildFonts } = require('./fonts');

    await buildFonts();

    // .css files are written standalone (in addition to _linked-fonts.{sg}.css),
    // matching the legacy compile pipeline that external consumers link to.
    expect(fs.existsSync(path.join(tmpDir, 'public', 'fonts', '_default', 'fonts.css'))).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, 'public', 'fonts', 'mobile', 'fonts.css'))).toBe(true);
  });

  it('substitutes $asset-host in self-hosted .css files when linked fonts are enabled', async () => {
    process.env.CLAYCLI_COMPILE_LINKED_FONTS = 'true';
    process.env.CLAYCLI_COMPILE_ASSET_HOST = 'https://cdn.example.com';
    ...
    const content = await fs.readFile(
      path.join(tmpDir, 'public', 'fonts', '_default', 'fonts.css'),
      'utf8'
    );
