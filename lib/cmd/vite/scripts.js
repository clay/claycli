...
exports.VITE_BOOTSTRAP_NO_MOUNT_KEY = VITE_BOOTSTRAP_NO_MOUNT_KEY;
exports.KILN_EDIT_ENTRY_KEY = KILN_EDIT_ENTRY_KEY;

/**
 * Normalize a `--only`/`options.only` value into a Set of build step names,
 * or null (meaning "no filter — run everything").
 *
 * Throws on any unrecognized step name rather than silently dropping it.
 * `cli/vite.js`'s own `--only` validation already rejects this at the CLI
 * layer, but this function is also reachable directly (`require('claycli')`
 * programmatic API), which bypasses that layer entirely — dropping an
 * unrecognized name here used to mean a typo silently fell through to
 * "requested.size === 0 → build everything", the opposite of what a
 * narrowed `--only` invocation is for.
 *
...
function normalizeRequestedSteps(only) {
  ...
  if (unrecognized.length > 0) {
    throw new Error(
      ...
      `Valid steps: ${['all', ...BUILD_STEP_NAMES].join(', ')}.`
    );
  }
...
function shouldRunMediaStep(shouldRun) {
  ...
  const requestedTemplatesOnly = shouldRun('templates') && !shouldRun('media');

  if (requestedTemplatesOnly) {
    console.log('[clay vite] --only templates also runs media (templates can inline public/media/**)');
  }
 ...
 *     nodeGlobals:        {},     // opt-in auto-injection for Node globals referenced as
 *                                  // BARE free identifiers (`Buffer.from(...)` with no
 *                                  // require('buffer')/import 'buffer') — Vite does not
 *                                  // polyfill these the way Browserify's insert-module-globals
 *                                  // did, and browser-compat.js's stubs only intercept an
 *                                  // explicit require()/import of the built-in. Narrow and
 *                                  // extensible: only `Buffer` is implemented today.
 *                                  //   nodeGlobals.Buffer === true       — @rollup/plugin-inject
 *                                  //     adds an auto-import of `Buffer` from 'buffer' ONLY to
 *                                  //     modules that reference the free identifier; resolves
 *                                  //     through the existing browser-compat.js buffer stub
 *                                  //     (which prefers a real globalThis.Buffer when present).
 *                                  //   nodeGlobals.Buffer === 'polyfill' — same auto-import, but
 *                                  //     `alias` is extended to point the 'buffer' specifier at
 *                                  //     the real npm `buffer` package instead of the stub, for
 *                                  //     byte-capable use (e.g. createHmac). Pulls in real bytes
 *                                  //     — use sparingly.
 *                                  //   false / unset (default) — no plugin added, zero bytes,
 *                                  //   zero behavior change.
 *     serverOnlyPackages: [],      // opt-in stopgap: package-path prefixes (e.g. 'amphora/lib')
 *                                  // that warn (not block) when pulled into a browser-reachable
 *                                  // bundle — for third-party server-only packages the
 *                                  // services/server -> services/client rewrite has no opinion
 *                                  // on. Defaults to [], a complete no-op. See service-rewrite.js
...
function getViteConfig(cliOptions = {}) {
  const config = {
    ...
    commonjsExclude:     [],
    browserStubs:        {},
    // nodeGlobals: { Buffer: true | 'polyfill' | false } — see the JSDoc block
    // above and buildPlugins()/baseViteConfig() below for the wiring.
    nodeGlobals:         {},
    // serverOnlyPackages: [] — see the JSDoc block above and
    // viteServiceRewritePlugin() in plugins/service-rewrite.js.
    serverOnlyPackages:  [],
 ...
 *                        in the <script> block.
 *
 *   5. bare-node-globals-scan — always present, report-only. Scans each module's
 *                        source for `Buffer` used as a free identifier (no
 *                        accompanying require/import of 'buffer') and adds it to
 *                        the end-of-build summary. Never alters output.
 *
 *   6. inject (Buffer) — opt-in, only added when bundlerConfig().nodeGlobals.Buffer
 *                        is truthy (see getViteConfig() JSDoc). @rollup/plugin-inject
 *                        auto-imports `Buffer` only into modules that reference it as
 *                        a bare identifier; absent by default (zero bytes, zero
 *                        behavior change).
 *
 *   7. user plugins    — site-specific overrides from bundlerConfig().plugins,
 ...
 * @param {object}   [opts.nodeGlobals]  bundlerConfig().nodeGlobals — see
 *   getViteConfig() JSDoc. Only `Buffer` is implemented today.
 * @param {string[]} [opts.serverOnlyPackages]  bundlerConfig().serverOnlyPackages
 *   — forwarded to viteServiceRewritePlugin; see that plugin's JSDoc.
 * @returns {object[]}
 */
function buildPlugins(extraPlugins = [], browserStubs = {}, opts = {}) {
  const plugins = [
    ...
    viteServiceRewritePlugin(opts.serverOnlyPackages || []),
    viteMissingModulePlugin(),
    viteVue2Plugin({ minify: opts.minify === true }),
    // Always present — see scanBareNodeGlobals()'s doc block. Report-only,
    // never alters output.
    viteBareNodeGlobalsPlugin(),
  ];

  // nodeGlobals.Buffer (true or 'polyfill') — opt-in auto-import of the bare
  // `Buffer` identifier. Absent by default: no plugin added, zero bytes, zero
  // behavior change. See getViteConfig() JSDoc for the two modes; the choice
  // between the browser-compat stub and the real npm `buffer` package is made
  // via `alias` in baseViteConfig(), not here.
  if (opts.nodeGlobals && opts.nodeGlobals.Buffer) {
    const inject = require('@rollup/plugin-inject');

    plugins.push(inject({ Buffer: ['buffer', 'Buffer'] }));
  }
 ...
 */
/**
 * Build the `resolve.alias` map for a build pass: the site's own
 * bundlerConfig().alias, plus (only for nodeGlobals.Buffer === 'polyfill') a
 * 'buffer' → real npm `buffer` package override. See getViteConfig() JSDoc
 * and the `alias` field in baseViteConfig() for full context.
 *
 * @param {object} [siteAlias]    viteCfg.alias
 * @param {object} [nodeGlobals]  viteCfg.nodeGlobals
 * @returns {object}
 */
function buildResolveAlias(siteAlias, nodeGlobals) {
  const alias = { ...siteAlias || {} };

  if (nodeGlobals && nodeGlobals.Buffer === 'polyfill') {
    // GOTCHA: require.resolve('buffer') returns the STRING 'buffer' — Node's
    // own built-in core module of that name shadows the npm package, so a
    // bare specifier resolves to the core module id, not a file path. That
    // makes the alias value a no-op (aliasing 'buffer' to 'buffer'). Forcing
    // a subpath resolves the npm package's real file on disk instead.
...
function baseViteConfig(viteCfg) {
  ...
  const nodeGlobals = viteCfg.nodeGlobals || {};
  ...
  return {
    ...
    plugins:    buildPlugins(viteCfg.plugins, viteCfg.browserStubs, {
      ...
      minify: viteCfg.minify === true,
      nodeGlobals,
      serverOnlyPackages: viteCfg.serverOnlyPackages || [],
    }),
    ...
    resolve: {
      ...
      //
      // nodeGlobals.Buffer === 'polyfill' (see getViteConfig() JSDoc) extends
      // this with a 'buffer' → real npm `buffer` package alias, so the
      // @rollup/plugin-inject auto-import above resolves to actual byte
      // operations instead of browser-compat.js's minimal stub. Vite resolves
      // aliases before any enforce:'pre' plugin (including browser-compat's
      // own 'buffer' interception) ever sees the specifier, so this alias
      // takes priority without needing to touch browser-compat.js itself.
      alias: buildResolveAlias(viteCfg.alias, nodeGlobals),
    },
 ...
 */
/**
 * Add the edit-mode no-mount bootstrap to a Rollup input map when it exists.
 *
 * It rides along in the view graph so Rollup dedupes the initializer chunks
 * both variants share instead of emitting a second copy.
 *
 * Guarded on existence so an older or stubbed generate-bootstrap — one that
 * predates this entry — still produces a working build.
 *
 * @param {object} input - Rollup input map, mutated in place
 * @returns {object} the same input map
 */
function withNoMountBootstrap(input) {
  if (VITE_BOOTSTRAP_NO_MOUNT_FILE && fs.existsSync(VITE_BOOTSTRAP_NO_MOUNT_FILE)) {
    input[VITE_BOOTSTRAP_NO_MOUNT_KEY] = VITE_BOOTSTRAP_NO_MOUNT_FILE;
  }
...
async function buildJS(options = {}) {
  ...
  // Both Rollup passes have resolved — print the aggregated MISSING_GLOBAL_NAME
  // summary (if any occurrences were collected) now that the build is done.
...
async function watch(options = {}) {
  ...
  const watchInput = withNoMountBootstrap({ [VITE_BOOTSTRAP_KEY]: VITE_BOOTSTRAP_FILE });

  if (viteCfg.kilnSplit) {
    watchInput[KILN_EDIT_ENTRY_KEY] = KILN_EDIT_ENTRY_FILE;
  }
...
const SUPPRESSED_WARNING_CODES = new Set([
  ...
  // NOTE: MISSING_GLOBAL_NAME is intentionally NOT in this set — see below.
]);

// ── MISSING_GLOBAL_NAME diagnostics ─────────────────────────────────────────
//
// A Node-builtin identifier (Buffer, process, global, ...) referenced as a
// BARE free variable — `Buffer.from(...)` with no accompanying
// require('buffer')/import 'buffer' — cannot be caught by
// plugins/browser-compat.js, which only intercepts an explicit require()/
// import of the built-in module. Vite does not auto-polyfill these globals
// (Browserify's insert-module-globals had no Vite equivalent; see
// `nodeGlobals` on getViteConfig() below for the opt-in fix once a real
// offender is found).
//
// Rather than blanket-suppress MISSING_GLOBAL_NAME (as before), collect every
// occurrence here and print one aggregated summary — see reportMissingGlobals(),
// called once buildJS()'s vite.build() calls resolve.
//
// IMPORTANT judgment call: verified by hand (a throwaway fixture referencing
// bare `Buffer`/`process`, built standalone with `vite.build()`) that Rollup's
// MISSING_GLOBAL_NAME is emitted ONLY for `dependency instanceof ExternalChunk
// && (format === 'umd' || format === 'iife')` (rollup/dist/shared/rollup.js,
// getRenderedDependencies()) — i.e. only when an external module has no entry
// in `output.globals` for a umd/iife build. This pipeline always builds
// `format: 'esm'` with no externals/globals, so in practice this warning does
// not fire for bare-global usage today (confirmed empirically: a bare-Buffer
// fixture built with format:'esm' produced zero warnings of any kind — Rollup
// leaves unresolved free identifiers in the output untouched, trusting a
// runtime global). The real warning shape, captured from an actual iife+
// external build, is `{ code, id, message, names: [guess], url }` — `id` is
// the external module's specifier and `names[0]` is Rollup's own guessed
// output.globals variable name (NOT the free identifier used in source).
// Collecting is still correct and forward-compatible (a plugin could manually
// warn with this code, or a future extraEntries build could use iife/umd),
// and strictly safer than the previous blanket suppression — it just means
// this exact code path won't be the mechanism that surfaces the known
// bare-Buffer bug.
//
// Because MISSING_GLOBAL_NAME doesn't fire for this pipeline, the actual
// working mechanism is scanBareNodeGlobals() below — a `transform` hook,
// added in buildPlugins(), that pattern-matches source text directly rather
// than relying on a Rollup warning. This is deliberately narrow (Buffer only,
// not process/global) and deliberately a heuristic (regex, not an AST walk):
//
//   - `process` is excluded: buildDefines() already substitutes
//     process.env / process.env.NODE_ENV / process.browser / process.version(s)
//     at build time, so almost every legitimate `process.` reference in this
//     codebase is already handled — flagging bare `process` would be mostly
//     false positives (every file that reads an env var).
//   - `global` is excluded: buildDefines() already rewrites it to `globalThis`.
//   - A module that already `require('buffer')`/imports from `'buffer'` is
//     skipped — that path already goes through browser-compat.js's stub;
//     re-flagging it here would be a false positive for correctly-written code.
//   - node_modules and virtual (`\0`-prefixed) module ids are skipped — this
//     is aimed at first-party source (components/services/global), where a
//     developer can actually act on the warning by editing the file named.
//
// A regex can't distinguish `Buffer` as a free identifier from `Buffer` as,
// say, an object property VALUE (`{ Buffer: 1 }`) or a string. It intentionally
// trades some false-positive risk for catching the real, reported bug shape
// (`Buffer.from(...)` with no accompanying import) without an AST dependency.
let missingGlobalOccurrences = [];

/**
 * Record one MISSING_GLOBAL_NAME occurrence for the end-of-build summary.
 * See reportMissingGlobals() and the doc block above for full context.
 *
 * @param {object} warning  Rollup warning object, `{ code, id, names, ... }`
 * @returns {void}
 */
function collectMissingGlobal(warning) {
  missingGlobalOccurrences.push({
    name: warning.names && warning.names[0] || warning.guess || 'unknown',
    id:   warning.id || warning.source || '(unknown module)',
  });
}

// Names scanned by scanBareNodeGlobals() — see the doc block above
// `missingGlobalOccurrences` for why `process`/`global` are deliberately
// excluded.
const BARE_GLOBAL_SCAN_NAMES = ['Buffer'];

// `require('buffer')`, `import 'buffer'`, `import { Buffer } from 'buffer'` —
// any form that already imports the module by name. A file matching this
// already goes through browser-compat.js's stub; scanning it too would flag
// correctly-written code.
const IMPORTS_BUFFER_RE = /(?:require\(\s*['"]buffer['"]\s*\)|\bfrom\s+['"]buffer['"])/;

/**
 * Scan one module's source for BARE_GLOBAL_SCAN_NAMES used as free
 * identifiers (not a property-access target, e.g. `foo.Buffer`) and record
 * each hit via collectMissingGlobal(). Regex-based, not an AST walk — see the
 * doc block above `missingGlobalOccurrences` for the accepted trade-offs.
 *
 * @param {string} code
 * @param {string} id
 * @returns {void}
 */
function scanBareNodeGlobals(code, id) {
  if (id.includes(`${path.sep}node_modules${path.sep}`) || id.startsWith('\0')) return;
  if (IMPORTS_BUFFER_RE.test(code)) return;

  for (const name of BARE_GLOBAL_SCAN_NAMES) {
    const re = new RegExp(`(?<![.\\w$])${name}(?![\\w$])`);

    if (re.test(code)) collectMissingGlobal({ names: [name], id });
  }
}

/**
 * Rollup plugin wrapping scanBareNodeGlobals() as a `transform` hook. Always
 * present (not opt-in) — it only ever reports via the existing
 * missingGlobalOccurrences summary, never alters the build, so there is no
 * behavior to opt into.
 *
 * @returns {object} Vite/Rollup plugin object
 */
function viteBareNodeGlobalsPlugin() {
  return {
    name: 'clay-vite-bare-node-globals-scan',
    transform(code, id) {
      scanBareNodeGlobals(code, id);
      return null;
    },
  };
}
...
function suppressWarning(warning, warn) {
  if (warning.code === 'MISSING_GLOBAL_NAME') {
    collectMissingGlobal(warning);
    return; // never fails the build — see reportMissingGlobals()
  }
...
/**
 * Print one aggregated, actionable console summary of every MISSING_GLOBAL_NAME
 * occurrence collected since the last call, then clear the list.
 *
 * Called once per build, after both Rollup passes (view + kiln) resolve — see
 * buildJS(). Console output only; this must never throw or fail the build.
 *
 * @returns {void}
 */
function reportMissingGlobals() {
  if (!missingGlobalOccurrences.length) return;

  try {
    const byName = new Map();

    for (const { name, id } of missingGlobalOccurrences) {
    ...
    console.log(
      `\n[clay vite] ${names.length} Node global(s) referenced as bare identifiers in browser-reachable code\n` +
      '(no polyfill is provided — see CLAY-VITE.md § Node globals in browser bundles):'
    );

    for (const name of names) {
      console.log(`  ${name}:`);
      for (const id of [...byName.get(name)].sort()) {
        console.log(`    - ${id}`);
      }
    }
    ...
    console.warn(`[clay vite] failed to print Node global summary: ${e.message}`);
  } finally {
    missingGlobalOccurrences = [];
  }
}
