...
function buildPlugins(options, overrides) {
  ...
  // Replace process.env.* references — must be done before commonjs
...
function missingModulePlugin() {
  return {
    ...
    resolveId(id, importer) {
      // Only intercept relative imports from our generated entry files
      ...
      if (!exists) {
        console.warn(`[rollup] skipping missing module: ${id} (imported from ${path.relative(CWD, importer)})`);
        // Return external so Rollup doesn't error — the dynamic import()
        // will simply never resolve at runtime (same as esbuild behaviour)
