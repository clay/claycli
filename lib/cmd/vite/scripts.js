 ...
 * surface and are irrelevant here.  Returns {} when the file is absent — the
 ...
 *
 * Reproduces the legacy webpack `pack` behaviour — dotenv-webpack with
 * `{ path: './.env', systemvars: true, allowEmptyValues: true }` — which INLINED
 ...
 *     the build actually holds a value — a local container started with
 *     `env_file`, or any build run with .env in cwd — the value is inlined exactly
 ...
 *
 *  2. RUNTIME catch-all: `process.env` → `window.process.env`.  Any var NOT
 ...
 * more-specific key, so it wins over the catch-all) → `process.env.NODE_ENV ===
...
function buildClientEnvDefines() {
  ...
  for (const name of Object.keys(env)) {
    ...
    // Empty/undefined → leave to the runtime catch-all; never inline a clobbering "".
...
function buildDefines(userDefines = {}) {
  ...
  return Object.assign(
    ...
    // build env (dotenv-webpack parity) + a `process.env` → `window.process.env`
