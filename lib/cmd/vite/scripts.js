...
const { writeManifest, manifestPlugin } = require('./manifest');
...
function buildPlugins(options, overrides) {
  ...
  // User-defined defines (e.g. DS → window.DS) are kept separate so they can
  // be applied via the custom transform hook that avoids replacing inside string
  // literals or function parameter lists.
  const userDefineMap = (overrides && overrides.define) || {};

  const replacePlugin = replace({
    values: builtinDefineMap,
    preventAssignment: true,
    delimiters: ['\\b', '\\b'],
  });

  // Custom transform-hook for:
  //   1. `global` → `globalThis`  (avoid corrupting path strings like "../global-nav/...")
  //   2. User-defined free-variable → window.X mappings (avoid replacing inside
  //      string literals like ['Eventify'] or function parameter names)
  //
  // Strategy: build a combined regex that only matches bare identifiers — i.e.
  // NOT preceded by . / ' " ` (property access or string/path context) and NOT
  // followed by / ' " ` (path/string context).
  const identifierDefines = Object.entries(userDefineMap);

  const globalReplacePlugin = {
    name: 'clay-global-replace',
    transform(code, id) {
      if (id.includes('/node_modules/')) return null;

      let modified = code;

      // Replace `global` → `globalThis` (bare identifier, not in path strings).
      // Lookbehind excludes . / ' " ` \w so we skip:
      //   - property accesses:  window.global
      //   - path strings:       require('./global'), import('../global-nav/...')
      //   - string quotes:      'global', "global"
      // Space and ( before `global` are allowed — they indicate a statement or
      // expression context (e.g. `var x = global`, `if (global)`, `return global`).
      if (modified.includes('global')) {
        modified = modified.replace(
          /(?<![./'"`\w])(?<!\brequire\()global(?![/'"`\w])/g,
          'globalThis'
        );
      }

      // Replace user-defined free variables (e.g. DS → window.DS).
      // Skip global/js/ files — they use dollar-slice DI injection which registers
      // these same names as local function parameters; replacing them there creates
      // invalid syntax (e.g. function(window.Eventify) is not valid JS).
      const isGlobalJs = id.includes('/global/js/');

      if (!isGlobalJs) {
        for (const [key, value] of identifierDefines) {
          if (!modified.includes(key)) continue;
          const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          // Only replace when not preceded by word char, ., ', ", ` (avoids
          // string literals and property accesses).
          const re = new RegExp('(?<![\\w.\'"` ])' + escapedKey + '(?![\\w\'"` ])', 'g');
...
/**
 * Minify all *.js files in outdir (including chunks/) in-place using esbuild.
 * Runs all transforms in parallel for speed.
 *
 * @param {string} outdir - Absolute path to the JS output directory
 * @returns {Promise<void>}
 */
async function minifyWithEsbuild(outdir) {
  const jsFiles = globSync(path.join(outdir, '**', '*.js'));
  ...
  await Promise.all(
    jsFiles.map(async (filePath) => {
      ...
      if (result.map) {
        await fs.writeFile(`${filePath}.map`, result.map, 'utf8');
      }
    })
  );
}
...
async function buildJS(options = {}) {
  ...
  const pass1Output = await pass1Bundle.write({
    ...
    manualChunks: manualChunksPlugin(minChunkSize),
    sourcemap: true,
  });
 ...
 *
 * @param {...object|null} outputs - Rollup OutputBundle objects
 * @returns {object}
 */
function mergeBundles(...outputs) {
  const merged = {};
...
async function watch(options = {}) {
  ...
  const rollupReady = (async () => {
    ...
    const watcher = rollup.watch({
      ...
      plugins: [
        ...plugins,
        manifestPlugin(DEST, '/js', (manifest) => {
...
function missingModulePlugin() {
  return {
    ...
    resolveId(id, importer) {
      ...
      // Strip virtual module suffixes added by @rollup/plugin-commonjs
      // e.g. "components/foo/client.js?commonjs-entry" → "components/foo/client.js"
    ...
    load(id) {
      if (id.startsWith('\0missing:')) {
        // Return a minimal CJS+ESM compatible stub so nothing crashes at runtime
        return 'export default undefined; module.exports = {};';
      }
      return null;
    },
  };
