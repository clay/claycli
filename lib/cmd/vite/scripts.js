...
async function watch(options = {}) {
  ...
  // Prints "[prefix] still building... (Xs)" every 3 s while a rebuild is in
  // flight so the terminal never looks frozen.  Returns a stop() function.
  function startProgressTick(prefix) {
    const t0 = Date.now();
    const id  = setInterval(() => {
      const s = Math.round((Date.now() - t0) / 1000);

      console.log(clr.changed(`${prefix} still building... (${s}s)`));
    }, 3000);
  ...
  const captureKilnOutputPlugin = {
    ...
    watchChange(id, { event }) {
      console.log(clr.changed(`[kiln] ${event}: `) + clr.file(path.relative(CWD, id)));
    },
  ...
  if (!viteCfg.kilnSplit) {
    ...
    async function handleKilnBundleEnd(event) {
      ...
      if (!isFirstKilnBuild && lastViewOutput) {
        // A kiln file changed while the view bundle was idle — update manifest.
        ...
        await writeManifest(manifest);
        await envCollector.write();
        console.log(clr.rebuilt('[kiln] Rebuilt') + ` (${moduleLabel} transformed${duration})`);
      }
    ...
    kilnWatcher.on('event', async (event) => {
      if (event.code === 'BUNDLE_START') {
        if (!isFirstKilnBuild) stopKilnTick = startProgressTick('[kiln]');
        else stopKilnTick = () => {};
        ...
        kilnOutputPromise = resetKilnOutputPromise();
        console.error(clr.error(`[kiln] Build error: ${event.error.message}`));
        if (event.result && event.result.close) event.result.close();
      }
    });
  }
  ...
  const captureOutputPlugin = {
    ...
    watchChange(id, { event }) {
      console.log(clr.changed(`[js] ${event}: `) + clr.file(path.relative(CWD, id)));
    },
  ...
  async function handleBundleEnd(event) {
    ...
    console.log(clr.rebuilt('[js] Rebuilt successfully') + ` (${moduleLabel} transformed${duration})`);
  ...
  watcher.on('event', async (event) => {
    if (event.code === 'BUNDLE_START') {
      ...
      stopJsTick = isFirstBuild ? () => {} : startProgressTick('[js]');
    } else if (event.code === 'BUNDLE_END') {
  ...
  const rebuildBootstrap = debounce(async (changedFile, eventType) => {
    ...
    if (!isChange) {
      console.log(clr.changed(`${isKilnFile ? '[kiln]' : '[js]'} ${eventType}: `) + clr.file(rel(changedFile)));
  ...
  const rebuildStyles = debounce((changedFile) => {
    ...
    const stopStyleTick = startProgressTick('[styles]');

    return buildStyles(buildOpts)
      .then(() => {
        stopStyleTick();
        console.log(clr.rebuilt(`[styles] Rebuilt (${Date.now() - start}ms)`));
      ...
      .catch(e => { stopStyleTick(); console.error(clr.error(`[styles] rebuild failed: ${e.message}`)); });
  }, 200);
  ...
  const rebuildFonts = debounce((changedFile) => {
    ...
    const start        = Date.now();
    const stopFontTick = startProgressTick('[fonts]');

    return buildFonts()
      .then(() => { stopFontTick(); console.log(clr.rebuilt(`[fonts] Rebuilt (${Date.now() - start}ms)`)); })
      .catch(e => { stopFontTick(); console.error(clr.error(`[fonts] rebuild failed: ${e.message}`)); });
  }, 200);
  ...
  const rebuildTemplates = debounce((changedFile) => {
    ...
    const start            = Date.now();
    const stopTemplateTick = startProgressTick('[templates]');

    return buildTemplates({ ...options, watch: true })
      .then(() => {
        stopTemplateTick();
        console.log(clr.rebuilt(`[templates] Rebuilt (${Date.now() - start}ms)`));
      ...
      .catch(e => { stopTemplateTick(); console.error(clr.error(`[templates] rebuild failed: ${e.message}`)); });
  }, 200);
