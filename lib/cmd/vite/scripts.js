...
async function watch(options = {}) {
  ...
  async function handleBundleEnd(event) {
    ...
    console.log(clr.rebuilt('[js] Rebuilt successfully') + ` (${moduleLabel} transformed)`);
  ...
  const rebuildBootstrap = debounce(async (changedFile, eventType) => {
    ...
    if (!isChange) {
      const prefix = isKilnFile ? '[kiln]' : '[js]';

      console.log(clr.changed(`${prefix} ${eventType}: `) + clr.file(rel(changedFile)));
    }
    ...
    if (!isChange && isGlobal) {
      ...
      await generateViteBootstrap();
    } else if (!isChange && changedFile.endsWith('client.js')) {
      // A new component client.js was added or removed: only the bootstrap's
      // component-discovery list needs updating.
      await generateViteBootstrap();
    } else if (!isChange && isKilnFile) {
      // A new model.js/kiln.js was added or removed: regenerate the kiln entry
      // so it includes/excludes the new file.  The kiln watcher detects the
      // entry file change and does an incremental rebuild automatically.
      await generateViteKilnEditEntry();
    }
    // For change events on kiln files, the kiln watcher's Rollup graph handles
    // the incremental rebuild — no manual trigger needed.
  }, 200);
  ...
  const rebuildStyles = debounce((changedFile) => {
    ...
    return buildStyles(buildOpts)
      .then(() => {
        console.log(clr.rebuilt('[styles] Rebuilt'));
  ...
  const rebuildTemplates = debounce((changedFile) => {
    ...
    return buildTemplates({ ...options, watch: true })
      .then(() => {
        console.log(clr.rebuilt('[templates] Rebuilt'));
