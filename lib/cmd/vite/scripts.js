...
async function watch(options = {}) {
  ...
  const rebuildBootstrap = debounce(async (changedFile, eventType) => {
    ...
    // Regenerate client-env.json when any JS file changes.
    // A developer might add a new process.env.SOME_VAR reference in any JS file;
    // if client-env.json is stale, amphora-html will not inject the variable into
    // the page and the component will see undefined at runtime — a silent failure
    // that is very hard to diagnose.
    try {
      await generateClientEnv();
    } catch (e) {
      console.error(clr.error(`[client-env] Failed to regenerate: ${e.message}`));
    }
  }, 200);
