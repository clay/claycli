...
async function watch(options = {}) {
  ...
  const rebuildBootstrap = debounce(async (changedFile, eventType) => {
    ...
    if (!isChange) {
      console.log(clr.changed(`[js] ${eventType}: `) + clr.file(rel(changedFile)));
    }
  ...
  const CSS_BUILD_ID = path.join(CLAY_DIR, 'css-build-id');
  ...
  const rebuildStyles = debounce((changedFile) => {
    ...
    return buildStyles(buildOpts)
      .then(() => {
        ...
        // Bump the CSS build ID so resolve-media.js appends ?v=<id> to CSS
        // URLs, busting the browser cache without requiring a server restart.
        return fs.ensureDir(CLAY_DIR)
          .then(() => fs.writeFile(CSS_BUILD_ID, String(Date.now())))
          .catch(() => {});
      })
