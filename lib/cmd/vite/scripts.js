...
function baseViteConfig(viteCfg) {
  ...
  return {
    ...
    // optimizeDeps is Vite's dev-server pre-bundler that converts CJS node_modules
    // to ESM before serving them to the browser via the dev server's module middleware.
    // It does NOT run during `vite build` — production builds go straight through
...
function suppressWarning(warning, warn) {
  ...
  // strictRequires:'auto'.  They become non-issues once the files are ESM.
  if (warning.code === 'CIRCULAR_DEPENDENCY') return;

  // CJS modules use `this` at the top level (equivalent to `module.exports`).
  // Rollup warns because in ESM `this` at the top level is undefined.
  // @rollup/plugin-commonjs wraps these so the `this` reference is correct.
  if (warning.code === 'THIS_IS_UNDEFINED') return;

  // Some global variables (e.g. jQuery's $) are expected to be on window and
  // are not imported.  Clay components reference them as free variables.
  if (warning.code === 'MISSING_GLOBAL_NAME') return;

  // Missing optional service or component imports are stubbed by
  // viteMissingModulePlugin.  Rollup warns about the unresolved path before
  // the plugin catches it; the warning is spurious.
  if (warning.code === 'UNRESOLVED_IMPORT') return;

  // CJS modules wrapped by @rollup/plugin-commonjs may not export named
  // bindings.  Callers that use `import { foo } from 'cjs-module'` get
  // undefined for missing named exports, which is the expected CJS behaviour.
  if (warning.code === 'MISSING_EXPORT') return;

  // eval() in node_modules (e.g. pyxis-frontend webpack dev bundle) is
  // intentional and cannot be removed.  Warn only for project source files.
