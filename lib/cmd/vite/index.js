 ...
 *
 * Defaults to the no-mount variant so component client.js files do not execute
 * while an editor is in Kiln — see generateViteBootstrap() for why that matters.
 * Pass mountInEditMode to restore the mounting bootstrap.
 *
 ...
 * @param {boolean} [mountInEditMode=false]
 * @returns {string[]}
 */
function getViteEditBootstrapScripts(assetPath, mountInEditMode = false) {
  if (mountInEditMode) return getViteViewScripts(assetPath);
 ...
 * @param {boolean} [options.mountInEditMode=false] - set true to mount component
 *   client.js in edit mode as well as view mode
 */
function resolveModuleScripts(media, assetPath, options) {
  if (!hasManifest()) return;

  const { edit = false, preloadEditBundle = false, mountInEditMode = false } = options || {};

  if (edit) {
    const editScripts      = getEditScripts(assetPath);
    const bootstrapScripts = getViteEditBootstrapScripts(assetPath, mountInEditMode);
