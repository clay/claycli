...
describe('copyVendor', () => {
  it('resolves clay-kiln from project cwd node_modules when using the default resolver', async () => {
    const kilnPkg = path.join(tmpDir, 'node_modules', 'clay-kiln', 'dist');
    await fs.ensureDir(kilnPkg);
    await fs.writeFile(path.join(kilnPkg, 'clay-kiln-edit.js'), '// fake edit bundle');
    await fs.writeFile(path.join(kilnPkg, 'clay-kiln-view.js'), '// fake view bundle');
