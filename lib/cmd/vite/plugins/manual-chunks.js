...
function manualChunksPlugin(minChunkSize = 4096, cwd = process.cwd()) {
  return function manualChunks(id, { getModuleInfo }) {
    ...
    // Never collapse entry points (e.g. components/*/client.js, .clay/_view-init).
    // They must remain separate chunks so _view-init's dynamic import() loaders
    // resolve to real chunk URLs and the mount runtime receives the correct module.
    if (info.isEntry) return undefined;
...
function findEntryId(startId, getModuleInfo, visited) {
  ...
  // Found an entry point — this is the chunk to assign to
  if (info.isEntry) return startId;

  // If this intermediate module has exactly one importer, keep walking up
  if (info.importers && info.importers.length === 1) {
    return findEntryId(info.importers[0], getModuleInfo, visited);
  }
