 ...
 */
/**
 * Returns true if a module qualifies as a "private dep" that can be inlined
 * into its single owning entry's chunk.
 *
 * @param {object} info               Rollup module info from getModuleInfo()
 * @param {number} manualChunksMinSize byte threshold
 * @returns {boolean}
 */
function isPrivateDep(info, manualChunksMinSize) {
  ...
  return importers.length === 1 && isSmall;
}
...
function viteManualChunksPlugin(manualChunksMinSize = 8192, cwd = process.cwd()) {
  return function manualChunks(id, { getModuleInfo }) {
    ...
    if (isPrivateDep(info, manualChunksMinSize)) {
      const owner = findOwnerEntry((info.importers || [])[0], getModuleInfo, new Set([id]));
