...
async function compileSfc(filename) {
  ...
  if (usesCjsExport) {
    // The <script> block used module.exports — @rollup/plugin-commonjs already
    // wrapped its internals, so we just need an ESM export here.  Using
    // `module.exports = __sfc__` at this point would leave a bare top-level
    // `module.exports` in the Rollup chunk, causing ReferenceError in browsers.
