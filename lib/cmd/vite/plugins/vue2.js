...
async function processStyleBlocks(ctx) {
  const { descriptor, compilerUtils, id, scopeId, parts, rawCss, warn, error, minify } = ctx;
...
function viteVue2Plugin(options = {}) {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      // ── Style blocks ───────────────────────────────────────────────────────
      await processStyleBlocks({ descriptor, compilerUtils, id, scopeId, parts, rawCss, warn: this.warn.bind(this), error: this.error.bind(this), minify });
