...
function viteBrowserCompatPlugin(customStubs = {}, options = {}) {
   ...
   * @returns {{pkgDir: string, fieldMap: Object|null} | null}
   */
  function findPackageBrowserField(fromDir) {
    ...
    while (dir && dir !== path.dirname(dir)) {
      ...
      if (noPkgDirs.has(dir)) { dir = path.dirname(dir); continue; }

      const pkgPath = path.join(dir, 'package.json');

      if (fs.existsSync(pkgPath)) {
        let entry = { pkgDir: dir, fieldMap: null };

        try {
          const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));

          if (pkg.browser && typeof pkg.browser === 'object' && !Array.isArray(pkg.browser)) {
            entry = { pkgDir: dir, fieldMap: pkg.browser };
          }
        } catch (_) {
  ...
  function isBrowserFieldFalseMapping(id, importer) {
    ...
    // 1. exact match on the raw import string (handles bare specifiers and
    //    exact relative strings copied from package.json).
    if (Object.prototype.hasOwnProperty.call(map, id) && map[id] === false) return true;

    // 2. resolved-relative-to-package match (handles sub-path browser entries
    //    when the importer uses a different relative prefix).
    if (id.startsWith('.')) {
      ...
      for (const key of Object.keys(map)) {
        ...
        const keyNoExt = key.replace(/\.js$/, '');

        if (keyNoExt === relNoExt) return true;
      }
    }
  ...
  return {
    ...
    load(id) {
      // Lenient externalize: short-circuit Vite's own
      // `__vite-browser-external` virtual before its load hook runs.
      // Returning a non-null string here stops Rollup's load pipeline, so
      // Vite's default throwing-proxy loader never executes.
      //
      // We intercept both the bare id and any `:<moduleName>` suffixed
      // variant. The plugin is registered with `enforce: 'pre'`, so this
      // hook runs before Vite's built-in resolve plugin and wins.
      if (lenientExternalize && (id === VITE_BROWSER_EXTERNAL || id.startsWith(`${VITE_BROWSER_EXTERNAL}:`))) {
