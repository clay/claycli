...
describe('vite scripts', () => {
  ...
  it('buildDefines inlines a non-empty build env var as a literal (dotenv-webpack parity)', async () => {
    await setupTmp('claycli-vite-scripts-clientenv-');

    const prev = process.env.PYXIS_HOST;

    // When the build env holds a value (e.g. a local container started with
    // env_file), it is baked in as a string literal — exactly as the legacy
    // dotenv-webpack({ systemvars: true }) pipeline did.
    ...
    try {
      ...
      // The runtime catch-all is always present for vars the build can't see.
  ...
  it('buildDefines leaves an empty/absent build env var to the runtime catch-all (no clobber)', async () => {
    await setupTmp('claycli-vite-scripts-clientenv-empty-');

    const prev = process.env.PYXIS_HOST;

    // An empty build value must NOT be inlined — otherwise `process.env.PYXIS_HOST`
    // folds to "" and master's `window.process.env.X = process.env.X` would clobber
    // the value _env-init.js hydrated at runtime.  It must fall through to the
    // `process.env` → `window.process.env` catch-all instead.
    process.env.PYXIS_HOST = '';
