...
describe('vite scripts', () => {
  ...
  it('buildJS runs split builds, writes manifest and client-env', async () => {
    ...
    const navClientFile = path.join(tmpDir, 'components', 'global-nav', 'client.js');
    ...
    const viewResult = {
      output: [
        ...
        // Dynamic entry: a component client.js loaded via import(). Must be
        // keyed in the manifest so the server can preload on-page components.
        {
          type: 'chunk',
          isEntry: false,
          isDynamicEntry: true,
          facadeModuleId: navClientFile,
          fileName: 'chunks/global-nav-client-222.js',
          imports: ['chunks/shared-111.js'],
        },
        // Plain shared chunk (no facade, not an entry): must NOT be keyed.
        {
          type: 'chunk',
          isEntry: false,
          isDynamicEntry: false,
          facadeModuleId: null,
          fileName: 'chunks/shared-111.js',
          imports: [],
        },
      ],
    };
    ...
    // Dynamic component entries are keyed so the server can preload them.
    expect(manifest['components/global-nav/client'].file).toBe('/js/chunks/global-nav-client-222.js');
    expect(manifest['components/global-nav/client'].imports).toEqual(['/js/chunks/shared-111.js']);
    // Plain shared chunks (no facadeModuleId) are not manifest keys.
    expect(Object.keys(manifest)).not.toContain('chunks/shared-111');
  });
