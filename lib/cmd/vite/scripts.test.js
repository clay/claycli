...
describe('vite scripts', () => {
  ...
  it('buildDefines inlines client-env vars from client-env.json at build time', async () => {
    await setupTmp('claycli-vite-scripts-clientenv-');
    await fs.writeJson(path.join(tmpDir, 'client-env.json'), ['PYXIS_HOST', 'NODE_ENV']);

    const prev = process.env.PYXIS_HOST;

    process.env.PYXIS_HOST = 'https://pyxis.example.test';

    try {
      const { buildDefines } = require('./scripts');
      const define = buildDefines();

      // PYXIS_HOST is inlined as a string literal (matches dotenv-webpack), so a
      // CJS `const HOST = process.env.PYXIS_HOST` can never compile to the empty
      // pVt={} stub the commonjs transform would otherwise produce.
      expect(define['process.env.PYXIS_HOST']).toBe(JSON.stringify('https://pyxis.example.test'));
      // NODE_ENV stays owned by buildDefines (not double-defined via client-env).
      expect(define['process.env.NODE_ENV']).toBeDefined();
    } finally {
  ...
  it('buildDefines falls back to all env on a cold build and skips invalid identifiers', async () => {
    await setupTmp('claycli-vite-scripts-coldenv-');
    // No client-env.json on a fresh CI checkout — must still inline so plugins
    // are not broken in production.

    const prev = process.env.CLAY_TEST_COLD_ENV;

    process.env.CLAY_TEST_COLD_ENV = 'cold-value';
    ...
    try {
      ...
      expect(define['process.env.CLAY_TEST_COLD_ENV']).toBe(JSON.stringify('cold-value'));
      // Keys that are not valid `process.env.<identifier>` members are skipped so
      // esbuild's define parser never sees a malformed expression.
      expect(define['process.env.clay.invalid-key']).toBeUndefined();
    } finally {
      delete process.env['clay.invalid-key'];
      if (prev === undefined) delete process.env.CLAY_TEST_COLD_ENV;
      else process.env.CLAY_TEST_COLD_ENV = prev;
    }
  });
  ...
  it('buildAll honors --only styles by skipping other steps', async () => {
    await setupTmp('claycli-vite-scripts-only-styles-');

    const order = [];

    jest.doMock('./media', () => ({ copyMedia: jest.fn().mockImplementation(async () => order.push('media')) }));
    jest.doMock('./styles', () => ({
      buildStyles: jest.fn().mockImplementation(async () => order.push('styles')),
      SRC_GLOBS: [],
    }));
    jest.doMock('./fonts', () => ({
      buildFonts: jest.fn().mockImplementation(async () => order.push('fonts')),
      FONTS_SRC_GLOB: '',
    }));
    jest.doMock('./templates', () => ({
      buildTemplates: jest.fn().mockImplementation(async () => order.push('templates')),
      TEMPLATE_GLOB_PATTERN: '*.hbs',
    }));
    jest.doMock('./vendor', () => ({ copyVendor: jest.fn().mockImplementation(async () => order.push('vendor')) }));
    jest.doMock('vite', () => ({
      build: jest.fn().mockResolvedValue({ output: [] }),
    }));
    jest.doMock('./plugins/client-env', () => ({
    ...
    await buildAll({ only: ['styles'] });
    restoreTTY();

    expect(order).toEqual(['styles']);
  });

  it('buildAll auto-runs media when --only templates is selected', async () => {
    await setupTmp('claycli-vite-scripts-only-templates-');
    ...
    await buildAll({ only: ['templates'] });
    restoreTTY();

    expect(order).toEqual(['media', 'templates']);
  });
