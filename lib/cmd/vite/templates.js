 ...
 */
async function inlineRead(source, filepath) {
  ...
  await Promise.all(matches.map(async (match) => {
    ...
    try {
      const contents = escape(await fs.readFile(filePath, 'utf8'));
...
/**
 * Compile one Handlebars template source file and return its { name, code } pair.
 *
 * @param {string}  srcPath  - Absolute path to the .hbs source file
 * @param {object}  hbs      - clayhandlebars instance (hbs = clayHbs())
 * @param {object}  clayHbs  - The clayhandlebars factory (for wrapPartial)
 * @param {boolean} minify   - When true, uglify the precompiled output
 * @returns {Promise<{name:string, code:string}>}
 */
async function compileTemplate(srcPath, hbs, clayHbs, minify) {
  const name = path.basename(path.dirname(srcPath));

  let source = await fs.readFile(srcPath, 'utf8');

  if (!srcPath.includes('clay-kiln')) {
    source = await inlineRead(source, srcPath);
  }
  ...
  if (minify) {
    ...
    output = result.code.slice(0, -1);
  }

  return { name, code: `window.kiln.componentTemplates['${name}']=${output}\n` };
}
...
async function buildTemplates(options = {}) {
  ...
  const compiled = (await Promise.all(
    templateFiles.map(srcPath => limit(async () => {
      try {
        const result = await compileTemplate(srcPath, hbs, clayHbs, minify);
  ...
  return writeOutput(compiled, minify);
}

/**
 * Write compiled templates to disk.
 * In minified mode writes bucketed files; otherwise writes one file per template.
 *
 * @param {Array}   compiled - Array of { name, code } objects
 * @param {boolean} minify
 * @returns {Promise<string[]>} list of output file paths written
 */
async function writeOutput(compiled, minify) {
  const outputPaths = [];
