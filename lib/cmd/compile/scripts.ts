...
/**
 * check if error list contains any fatal (non-asset) JS compile errors
...
function buildScripts(entries: string[], options: any = {}) {
  ...
  return new Promise((resolve) => {
    webpack(config, async (err: any, stats: any) => {
      ...
      // Fatal JS errors: skip all processing and file writes
      if (hasFatalErrors(ctx.errors)) {
        return resolve(ctx.errors);
      }
