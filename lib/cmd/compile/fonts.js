...
const _ = require('lodash'),
  ...
  fontFormats = ['woff', 'woff2', 'otf', 'ttf', 'css'],
  variables = {
    ...
    files: process.env.CLAYCLI_COMPILE_FILES_FONTS
  };
...
function getFontCSS(file, styleguide, isInlined) {
  ...
  if (ext === '.css') {
    ...
    if (isInlined) {
      css += `src: url(data:font/${format};charset=utf-8;base64,${file.contents.toString(
        'base64'
      )}) format("${format}"); }`;
    } else {
 ...
 * @param {boolean} [options.files] handle font css files
...
function compile(options = {}) {
  let styleguides = afs.getFolders(sourcePath),
    ...
    files = options.files || variables.files || false,
  ...
  function buildPipeline() {
    ...
    let tasks = _.reduce(styleguides, (streams, styleguide) => {
      let fontsSrc = path.join(sourcePath, styleguide, 'fonts', `*.{${fontFormats.join(',')}}`),
        inlinedFontsTask, linkedFontsTask, fileFontsTask;

      if (files) {
        fileFontsTask = gulp.src(fontsSrc)
          .pipe(newer({ dest: path.join(destPath, 'css', `_file-fonts.${styleguide}.css`), ctime: true }))
          .pipe(es.mapSync((file) => getFontCSS(file, styleguide)))
          .pipe(concat(`_file-fonts.${styleguide}.css`))
