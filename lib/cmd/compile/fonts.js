 ...
 * @param {boolean} [options.minify] minify
...
function compile(options = {}) {
    ...
    let tasks = _.reduce(styleguides, (streams, styleguide) => {
      ...
      if (inlined) {
        ...
        inlinedFontsTask = gulp.src(fontsSrc)
          ...
          .pipe(newer(path.join(destPath, 'css', `_inlined-fonts.${styleguide}.css`)))
      ...
      if (linked) {
        ...
        linkedFontsTask = gulp.src(fontsSrc)
          ...
          .pipe(newer(path.join(destPath, 'css', `_linked-fonts.${styleguide}.css`)))
  ...
  if (watch) {
    gulp.watch(path.join(sourcePath, '**', 'fonts', `*.{${fontFormats.join(',')}}`), gulp.task('fonts'));
  }
