...
describe('compile/scripts', () => {
    ...
    it('all output files are under public/js', () => {
      ...
      testIds.forEach((dep) => {
        ...
        if (Array.isArray(result)) {
          result.forEach((p) => expect(p).toContain(path.join('public', 'js')));
        } else {
  ...
  describe('rewriteServiceRequire', () => {
    ...
    it('returns a browserify transform', () => {
      const transform = scripts.rewriteServiceRequire();

      // Browserify transforms are through streams
