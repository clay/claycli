...
function compile(options = {}) {
  const componentsSrc = afs.getComponents().map((comp) => ({ name: comp, path: path.join(afs.getComponentPath(comp), 'media', mediaGlobs) })),
    ...
    sitesSrc = afs.getFolders(path.join(process.cwd(), 'sites'))
      .reduce((sites, site) => {
        // adds multiple records for sites with subsites depending on existing media directories
        // [ { name: site, path: sitePath }, { name: site/subsite, path: sitePath }, { name: site/subsite, path: subsitePath }]
        // workflow for subsites: 1. copy parent into subsite directory, 2. copy subsite into subsite directory, overriding parent files
        sites.push({ name: site, path: path.join(process.cwd(), 'sites', site, 'media', mediaGlobs) });
        _.each(afs.getFolders(path.join(process.cwd(), 'sites', site, 'subsites')), (subsite) => {
          // copy parent media assets to subsite dir
          sites.push({ name: `${site}/${subsite}`, path: path.join(process.cwd(), 'sites', site, 'media', mediaGlobs) });
          // if subsite has its own media, copy them after to override any parent files
          if (afs.fileExists(path.join(process.cwd(), 'sites', site, 'subsites', subsite, 'media'))) {
            sites.push({ name: `${site}/${subsite}`, path: path.join(process.cwd(), 'sites', site, 'subsites', subsite, 'media', mediaGlobs) });
          }
        });
        return sites;
      }, []);
