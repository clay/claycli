...
function compile(options = {}) {
  ...
  gulp.task('media', () => {
    ...
    return h(es.merge(componentTasks.concat(layoutsTask, styleguidesTask)));
  });

  if (watch) {
    let watchPaths = _.map(componentsSrc, (component) => component.path)
      .concat(_.map(layoutsSrc, (layout) => layout.path), _.map(styleguidesSrc, (styleguide) => styleguide.path));
