    ...
    if (minify) {
      ...
      return fs.stat(targetPath).then((targetStat) => {
        if (sourceFile.stat && sourceFile.stat.ctime > targetStat.ctime) {
          ...
          if (_.some(sourceFiles, (sFile) => sourceFile.stat && sourceFile.stat.ctime > getFileCtime(sFile))) {
            stream.push(sourceFile);
          }
        }
      }).catch(() => {
...
function compile(options = {}) {
  ...
  gulp.task('templates', () => {
    ...
    //       .pipe(gulpIf(minify, newer({ dest: path.join(destPath, 'css', `_inlined-fonts.${styleguide}.css`), ctime: true })))
    //       .pipe(changed(destPath, {
    //         transformPath: helpers.transformPath('_templates', destPath, minify),
