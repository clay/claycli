...
/**
 * get all kiln files (for edit mode)
 * @param  {boolean} minify
 * @return {array}
 */
function getAllKilns(minify) {
  if (minify) {
    // return ['_kiln-a-d', '_kiln-e-h'...]
    return glob.sync(path.join(destPath, '_kiln-?-?.js')).map((filepath) => path.parse(filepath).name);
  } else {
    // return ['ad.model', 'article.model'...]
    return glob.sync(path.join(destPath, '*.kiln.js')).map((filepath) => path.parse(filepath).name);
  }
}
...
function getDependencies(scripts, assetPath, options = {}) {
  ...
  if (edit) {
    return _.flatten([
      ...
      getAllKilns(minify), // model.js files
