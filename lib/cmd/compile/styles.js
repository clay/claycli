...
const _ = require('lodash'),
  ...
  srcPath = path.join(process.cwd(), 'styleguides', '**', 'components', '*.css'),
  ...
  variables = {
    ...
    'asset-host': process.env.CLAYCLI_COMPILE_ASSET_HOST,
...
function compile(options = {}) {
  ...
  gulp.task('styles', () => {
    return h(gulp.src(srcPath)
      // if we're minifying (but not watching for incremental changes),
      // recompile ALL files instead of just updated files
      .pipe(gulpIf(!minify || watch, changed(destPath, {
        transformPath,
        hasChanged
      })))
  ...
  if (watch) {
    return {
      ...
      watch: gulp.watch(srcPath, gulp.task('styles'))
    };
  } else {
    return {
      build: gulp.task('styles')(),
