...
function compile(options = {}) {
  const watch = options.watch || false,
    ...
    globs = options.globs || [],
    // client.js, model.js, and kiln plugins are passed to megabundler
    bundleEntries = glob.sync(clientGlob).concat(glob.sync(modelGlob), glob.sync(kilnPluginsGlob)),
    // options are set beforehand, so we can grab the cached files to watch afterwards
    bundleOptions = { minify },
    // start by watching megabundled entries (client, model, kiln plugins), kiln files, and legacy _global.js
    watcher = watch && chokidar.watch(bundleEntries.concat([kilnGlob], globs));
    ...
    build: h(buildScripts(bundleEntries, bundleOptions).then((res) => {
      if (watcher) {
        ...
        watcher.on('change', (file) => {
          ...
          buildKiln();
          // todo: kick off rebuild of legacy _global.js scripts
        });
      }
      return res;
