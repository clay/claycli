 ...
 * @param {array} legacyGlobs
 * @return {string|undefined} module id
 */
function getModuleId(file, legacyGlobs) {
  const name = file.split('/').slice(-2)[0],
    ...
    isKilnPlugin = _.includes(file, '/services/kiln/'),
    // everything passed in via legacy globs compiles to public/js/_global.js
    isLegacyFile = _.find(legacyGlobs, (g) => minimatch(file, g));

  if (isKilnPlugin) {
    return `${path.parse(file).name}.kilnplugin`;
  } else if (isLegacyFile) {
    return `${path.parse(file).name}.legacy`;
  } else if (_.endsWith(file, 'client.js')) {
    return `${name}.client`;
...
* @param {array}  [opts.legacyGlobs]
* @return {function} A function that returns an ID given a specified file
**/
function idGenerator({ cachedIds, legacyGlobs }) {
  ...
  return (file) => {
    let id = generatedIds[file] || (generatedIds[file] = getModuleId(file, legacyGlobs) || i++);
 ...
 */
function labeler(b, { cachedIds = {}, legacyGlobs }) {
  const getOrGenerateId = idGenerator({ cachedIds, legacyGlobs });
...
function getOutfile(dep) {
  ...
  if (_.includes(['prelude', 'postlude'], id)) {
    ...
    // legacy js is compiled to public/js/_global.js
    // and should be included on every page (in view mode)
    return path.join(destPath, '_global.js');
...
function compile(options = {}) {
  const watch = options.watch || false,
    ...
    bundleEntries = glob.sync(clientGlob).concat(glob.sync(modelGlob), glob.sync(kilnPluginsGlob), globs.length ? glob.sync(globs) : []),
    // options are set beforehand, so we can grab the cached files to watch afterwards
    bundleOptions = { minify, legacyGlobs: globs },
    // start by watching megabundled entries (client, model, kiln plugins, legacy _global.js) and kiln files
    watcher = watch && chokidar.watch(bundleEntries.concat([kilnGlob]));
...
function getComputedDeps(entryIDs) {
  ...
  // compute deps for legacy _global.js
  computeDep('_global', out, registry);
 ...
 * @return {array}
 */
function getDependencies(scripts, assetPath, { edit = false, minify = false }) {
  if (edit) {
    return _.flatten([
      '_prelude',
    ...
    return _.flatten([
      '_prelude',
      getComputedDeps(entryIDs, minify), // dependencies for client.js and _global.js
      '_global.js',
      '_postlude'
    ]).map((id) => idToPublicPath(id, assetPath));
  }
