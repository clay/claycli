...
const _ = require('lodash'),
  ...
  clientCachePath = path.resolve(process.cwd(), 'browserify-client-cache.json'), // make sure this is gitignored!
  modelCachePath = path.resolve(process.cwd(), 'browserify-model-cache.json'), // make sure this is gitignored!
...
function rewriteRequire(moduleId, rewrite) {
  ...
  return transformTools.makeRequireTransform('requireTransform',
    ...
    ([filepath], {file}, cb) => {
      const absolutePath = resolve(filepath, { basedir: path.dirname(file) });
  ...
  return transformTools.makeRequireTransform('requireTransform',
    ...
    ([filepath], {file}, cb) => {
      var parsedRequire = path.parse(filepath), // parse the require path
        ...
        isServerSideService = _.endsWith(absoluteRequirePath, '/services/server'), // Does it lead to the server-side service?
        absoluteClientPath = path.resolve(absoluteRequirePath, '../../services/client', parsedRequire.name);

      // test if the client-side version of the service exists. If it doesn't then this task
      ...
      if (isServerSideService && !fs.existsSync(`${absoluteClientPath}.js`)) {
        throw new Error('A server-side service used by model.js must have a client-side counterpart');
      }

      // map the server-side service to the client-side equivalent
...
function buildViewModeScripts(entries, options = {}) {
  ...
  if (!options.minify) {
    browserifyCache(bundler, { cacheFile: clientCachePath });
 ...
 * @return {Stream}
 */
function buildEditModeScripts(entries, options = {}) {
  const bundler = browserify({
      dedupe: false,
      // cache and packageCache are used by browserify-cache-api to speed up full rebuilds
      cache: {},
  ...
  // speed up full rebuilds for developers
  if (!options.minify) {
    browserifyCache(bundler, { cacheFile: modelCachePath });
    // note: this file is NOT written in production environments
  }
  ...
  bundler.require(entries)
    ...
    // write out browser-pack chunks so module chunks can be concatenated arbitrarily
    .plugin(browserifyGlobalPack, {
      getOutfile: getOutfile('_model-deps'),
      cache: options.cache.path
    })
...
function compile(options = {}) {
  const watch = options.watch || false,
    ...
    globs = options.globs || [],
    clientEntries = glob.sync(clientGlob),
    modelEntries = glob.sync(modelGlob),
    ...
    //   // h(buildEditModeScripts(modelEntries, bundleOptions)).flatten()
    // ]).flatten();
  ...
  return {
    build: h(buildEditModeScripts(modelEntries, bundleOptions)).flatten(),
    watch: null
  };
  ...
  //       viewModeWatcher.add(bundleOptions.cache.ids);
  //       return results;
  //     })), buildKiln()]).flatten(),
  ...
  // } else {
  //   return {
  //     build: buildViewModeScripts(clientEntries, bundleOptions),
  //     watch: null
  //   };
  // }
