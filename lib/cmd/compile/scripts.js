...
const _ = require('lodash'),
  ...
  gulpEsbuild = require('gulp-esbuild'),
  vuePlugin = require('esbuild-vue'),
  stylePlugin = require('esbuild-style-plugin'),
...
function buildScripts(entries, options = {}) {
  ...
  // entries (all client.js, model.js, kiln.js files)
  ...
  bundler.require(entries)
    ...
    .plugin(extractCSS, { out: kilnPluginCSSDestPath }) // REWRITE TO USE ESBUILD
    // remove unreachable code branches
    .transform(unreachableCodeTransform) // DO NOT NEED
    // map services/server to services/client
    .transform(rewriteServiceRequire()) // ADAPTER?
    // assign each module a module ID, defaulting to old module IDs in cache
    .plugin(labeler, { cachedIds: options.cache.ids, legacyFiles: options.legacyFiles }) // SHOULD NOT NEED - INCREMENTAL BUILD
    // keep only entry (changed) files and new files; do not process existing, unchanged files
    .plugin(filterUnchanged, { cachedFiles: options.cache.files }) // SHOULD NOT NEED - INCREMENTAL BUILD
    ...
    .plugin(browserifyExtractIds, { // SHOULD NOT NEED - INCREMENTAL BUILD
      callback(err, ids) {
        if (err) {
          return console.error(err);
        }
    ...
    .plugin(transformEnv, { // REWRITE TO USE ESBUILD
      callback: (err, env) => {
        if (err) {
          return console.error(err);
        };
    ...
    .plugin(browserifyGlobalPack, { // ?? NEED TO REVIEW RENDER
      getOutfile,
      cache: options.cache.path
    })
    // shorten bundle size by rewriting require() to use module IDs
    .plugin(bundleCollapser); // ?? NEED TO REVIEW RENDER

  if (options.minify) { // DO NOT NEED
    bundler.transform(uglifyify, { global: true, output: { inline_script: true }});
  }

  return new Promise((resolve) => {
    bundler.bundle()
      .on('end', () => { // SHOULD NOT NEED - INCREMENTAL BUILD
...
function compile(options = {}) {
  const watch = options.watch || false,
    ...
    watcher = watch && chokidar.watch(bundleEntries); // REMOVE

  // make sure public/js exists
  fs.ensureDirSync(destPath);

  return {
    build: h(buildScripts(bundleEntries, bundleOptions)).flatten().append(buildKiln()).append(copyClientInit()),
    watch: watcher // REMOVE
  };
}
 ...
 * - what deps / things are we able to change?
 *
 * - validation / testing -> how will we know the builds are working / the same as before?
 *
 * - remove highland
 * - start using esbuild plugin
 *
 */
