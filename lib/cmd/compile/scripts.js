...
const _ = require('lodash'),
  ...
  postcssChain = require('../../postcss-chain'),
  // globbing patterns
...
/**
 * Build the PostCSS plugin chain applied to every Vue <style> block, via the
 * shared lib/postcss-chain.js resolver (used by BOTH this pipeline and
 * `clay vite`'s Vue 2 SFC plugin — lib/cmd/vite/plugins/vue2.js — so the two
 * can't silently drift apart; see that module's own doc comment for why).
 *
 * `@nymag/vueify` hard-pins `postcss@^7` and runs this chain through its OWN
 * internal postcss instance, so an incompatible-major plugin here would
 * throw "PostCSS plugin X requires PostCSS Y" the first time a Kiln plugin
 * style compiles — resolveKilnPostcssChain() validates for exactly this and
 * reports (rather than silently drops) anything missing or incompatible.
 *
 * @returns {Array}
 */
function buildVuePostcssChain() {
  const chain = postcssChain.resolveKilnPostcssChain({
    cwd: process.cwd(),
    pluginArgs: {
...
function buildScripts(entries, options = {}) {
  ...
  const skipVueStylePostcss = process.env.CLAYCLI_COMPILE_SCRIPTS_SKIP_STYLE_POSTCSS === 'true';
  const vueifyTransformOptions = {
    babel: babelConfig
  };
  ...
  //
  // Resolved + validated by the shared lib/postcss-chain.js (see
  // buildVuePostcssChain's own doc comment above) against the HOST
  // project's node_modules only — never claycli's own bundled copies,
  // which would crash @nymag/vueify's internal postcss@7 instance if handed
  // a postcss8-era plugin. Anything missing or incompatible is reported via
  // console.warn with the exact fix, and simply excluded from the chain.
  ...
  bundler.require(entries)
    ...
    .transform(vueify, vueifyTransformOptions)
...
function compile(options = {}) {
  ...
  // vueify extract-css plugin writes kiln plugin styles to public/css/_kiln-plugins.css
  // and fails on clean environments unless the directory exists first.
