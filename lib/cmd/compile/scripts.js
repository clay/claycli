...
function idGenerator({ cachedIds }) {
  ...
  return (file) => generatedIds[file] || (generatedIds[file] = getModuleId(file) || i++);
}
...
function getOutfile(prefix) {
  return (dep) => {
    ...
    if (_.endsWith(id, '.model')) {
      ...
      // model deps are compiled to <number>.js and _model-deps-<letter>-<letter>.js
      return [
        path.join(destPath, `${id}.js`),
        path.join(destPath, `_client-deps-${helpers.bucket(id)}.js`)
      ];
    } else if (_.endsWith(id, '.client')) {
      // client.js files are compiled to <name>.client.js
      return path.join(destPath, `${id}.js`);
    } else if (prefix === '_client-deps') {
      // client deps are compiled to <number>.js and _client-deps-<letter>-<letter>.js
      return [
        path.join(destPath, `${id}.js`),
        path.join(destPath, `_client-deps-${helpers.bucket(id)}.js`)
      ];
    } else {
      console.log('Not sure where to put this file: ' + id);
    }
  };
}
...
function buildViewModeScripts(entries, options = {}) {
  ...
  // begin the bundling stream
  b = bundler.bundle().resume();
  ...
  return h(b, (b, cb) => {
    ...
    return () => {
      b.removeListener('end', cb);
      b.removeListener('close', cb);
      b.removeListener('error', cb);
    };
  }).map((chunk) => {
    console.log(chunk);
    return { type: 'info', message: chunk };
  });
...
function compile(options = {}) {
  ...
  if (watch) {
    ...
    viewModePromise.then((res) => h(res));
    return {
      build: h(viewModePromise).flatten(),
      watch: null
    };
  }
