 ...
 *
 * Mirrors the legacy vueify chain (cssImport → autoprefixer → mixins →
 * nested → simple-vars).
 *
 ...
 * the two — picking individual plugins across versions causes runtime
 ...
 *
 * Plugins that aren't installed in the chosen tree are dropped silently —
...
function buildVuePostcssChain() {
  ...
  const paths = useHost ? hostPaths : undefined; // undefined → claycli's own resolver
  const specs = [
