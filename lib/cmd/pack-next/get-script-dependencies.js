...
const { KILN_EDIT_ENTRY_KEY } = require('./build');
 ...
 * @returns {string[]}
 */
function collectScripts(key, manifest, seen, base) {
  const entry = manifest[key];

  if (!entry || seen.has(key)) return [];
  seen.add(key);

  const scripts = [entry.file.replace(/^\/js/, base || '/js')];
...
function getDependenciesNextForComponents(componentNames, assetPath, globalKeys) {
  ...
  // Always include explicitly requested global keys first
...
/**
 * Return an array of template script paths still served by clay compile
 * (unchanged from the Browserify pipeline).
 *
 * @returns {string[]}
 */
function getTemplatePaths() {
  return globSync(path.join(DEST, '*.template.js'))
