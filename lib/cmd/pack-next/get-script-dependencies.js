...
const { KILN_EDIT_ENTRY_KEY, VIEW_INIT_ENTRY_KEY } = require('./build');

const DEST = path.resolve(process.cwd(), 'public', 'js');
const MANIFEST_PATH = path.join(DEST, '_manifest.json');

/**
 * Read and return the _manifest.json written by `clay pack-next`.
...
function getDependenciesNext(scripts, assetPath, options) {
  ...
  if (!manifest) {
    throw new Error(
      'pack-next: _manifest.json not found.\n' +
      'Run `clay pack-next` first to generate the build output.'
    );
  }
...
/**
 * Return an array of hashed script URLs for the given component names (view mode).
 *
 * Used when amphora-html passes `locals._components` (an array of raw component
 * names like ['article', 'header']) instead of legacy public script paths.
 *
 * @param {string[]} componentNames - Component names from state._components
 * @param {string}   assetPath      - Site asset prefix
 * @param {string[]} [globalKeys]   - Extra manifest keys always loaded (e.g. global scripts)
 * @returns {string[]}
 */
function getDependenciesNextForComponents(componentNames, assetPath, globalKeys) {
  const manifest = readManifest();
  ...
  // When no per-component scripts are requested (view mode with _view-init handling
  // dynamic loading), use chunksOnly so we only include the entry file + shared
  // chunks for each global key — not the 220+ component client files that
  // esbuild lists as potential dynamic imports of _view-init in the manifest.
  const initsOnly = !componentNames || componentNames.length === 0;

  // Always prepend the generated view-init bundle first so the sticky events
  // shim and component mounting logic are in place before any GLOBAL_KEYS script
  // (e.g. aaa-module-mounting.js) runs.  chunksOnly=true avoids adding all
  // component client files as eager <script> tags — they are loaded on demand.
  if (manifest[VIEW_INIT_ENTRY_KEY]) {
    result.push(...collectScripts(VIEW_INIT_ENTRY_KEY, manifest, seen, base, true));
  }

  // Always include explicitly requested global keys after _view-init
  for (const key of (globalKeys || [])) {
    if (manifest[key]) {
      result.push(...collectScripts(key, manifest, seen, base, initsOnly));
    }
  }

  // Include client scripts for each component on the page
  for (const name of (componentNames || [])) {
    const clientKey = `components/${name}/client`;

    if (manifest[clientKey]) {
      result.push(...collectScripts(clientKey, manifest, seen, base));
    }
  }
 ...
 *
 * The aggregator (generated by generateKilnEditEntry in build.js) pre-populates
...
/**
 * Returns true when _manifest.json exists on disk, meaning pack-next output
...
/**
 * Return an array of template script paths produced by pack-next.
 *
