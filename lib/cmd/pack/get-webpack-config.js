...
function createClientConfig() {
  ...
  config
    ...
    const clientEnvFile = path.resolve(process.cwd(), 'client-env.json');

    if (afs.fileExists(clientEnvFile)) {
      const exposedEnvVars = afs.tryRequire(clientEnvFile);

      if (!exposedEnvVars) return;

      const defineMap = exposedEnvVars.reduce((envVars, envKey) => {
        envVars[`process.env.${envKey}`] = process.env[envKey];
        return envVars;
      }, {});

      config
        .plugin(DefinePlugin, [defineMap]);
    }
