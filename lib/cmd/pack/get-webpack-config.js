 ...
 *
 * @param {string} glob - The glob pattern against which to match scripts.
 * @returns {Object} - A mapping of Webpack entrypoints.
 */
function getEntries(glob) {
  return globSync(glob).reduce((entries, pathname) => {
    const namespace = path.relative(process.cwd(), path.dirname(pathname));
    const name = path.join(namespace, path.basename(pathname, path.extname(pathname)));
