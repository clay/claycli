 ...
 * @property {string[]} globs - A list of compilation entry points.
...
function createClientConfig(entryPoints) {
  ...
  // Expand globs passed to the CLI and use them as the build's entry points.
  globSync(entryPoints)
    .forEach(entryPoint => {
      config
        .entry('main')
        .add(entryPoint);
 ...
 */
function getWebpackConfig({ globs, environment = 'development' }) {
  let client = createClientConfig(globs); // TODO: Support serverside builds.
