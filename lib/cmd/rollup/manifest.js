...
async function writeManifest(rollupBundle, outdir, publicBase = '/js') {
  ...
  for (const [fileName, chunk] of Object.entries(rollupBundle)) {
    ...
    // @rollup/plugin-commonjs appends "?commonjs-entry" (and similar query
    // suffixes) to facadeModuleId for CJS entry points.  Strip the query
    // portion before deriving the manifest key so we get the same clean key
    // that esbuild produces (e.g. "components/article/client" not
    // "components/article/client.js?commonjs-entry").
    const cleanFacadeId = facadeId.replace(/\?.*$/, '');
    const entryKey = path.relative(process.cwd(), cleanFacadeId)
      .replace(/\\/g, '/')
      .replace(/\.js$/, '');
