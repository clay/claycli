...
function esbuildTransformPlugin(options = {}) {
  ...
  //
  // `process.env` is defined as `({})` so that any `process.env.FOO` access
  // that is not covered by a more-specific key returns `undefined` instead of
  // throwing a ReferenceError.  esbuild applies defines from most-specific to
  // least-specific, so the explicit keys below still win:
  //   process.env.NODE_ENV → "development"  (specific wins)
  //   process.env.FOO      → ({}).FOO = undefined  (fallback)
  //
  // Non-env `process.*` accesses (process.stdout, process.cwd, process.pid…)
  // only appear inside `isNode` guards (e.g. `typeof process !== 'undefined'
  // && process.versions.node != null`).  Our `process.versions → {}` define
  // makes `process.versions.node` evaluate to `undefined`, so `isNode = false`
  // and those branches are never taken in the browser.
  ...
  // Build a single regex that matches ANY of the define keys so we can skip
  // esbuild.transform() for files that don't reference any define target.
  // esbuild.transform is an async call to a native binary process; skipping it
  // for clean files (most node_modules, many utility modules) measurably reduces
  // total build time across large module graphs.
  //
  // Each key is escaped for use as a literal regex fragment.  For dotted keys
  // like "process.env.NODE_ENV" we only need to detect the root object
  // ("process"), so the regex checks for "process" and lets esbuild handle
  // the fine-grained replacement once we know the file is worth transforming.
  const defineRoots = [...new Set(
    Object.keys(defineMap).map(k => k.replace(/[.[].*/, '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
  )];
  const DEFINE_RE = new RegExp(`\\b(${defineRoots.join('|')})\\b`);

  return {
    name: 'clay-esbuild-transform',

    // --- define substitution -------------------------------------------------
    // Runs on every .js/.cjs/.mjs source module and on .vue files (after
    // vue2Plugin has compiled them to plain JS) to apply compile-time defines
    // such as process.env.NODE_ENV, __filename, __dirname, etc.
    ...
    async transform(code, id) {
      ...
      if (!/\.(js|cjs|mjs|vue)$/.test(id)) return null;

      // Skip files that don't contain any define target — avoids invoking the
      // esbuild binary for the majority of clean ESM/utility modules.
      if (!DEFINE_RE.test(code)) return null;
