 ...
 */
function lintURL(uri, isRecursive, prefix, concurrency) {
  let spinner;

  spinner = logger.startSpinner('Linting url...');
  clay.checkAllReferences(uri, isRecursive, prefix, concurrency)
...
function builder(yargs) {
  return yargs
    ...
    .example('$0 lint domain.com/_components/foo', 'lint foo data')
    .example('$0 lint -f components/foo', 'lint foo data and schema')
    .example('$0 lint domain.com/_components/layout -r', 'lint ALL components in layout')
    .example('$0 lint domain.com/_pages/123 -r', 'lint components in page and its layout')
...
function handler(argv) {
  if (argv.url) {
    return lintURL(argv.url, argv.recursive, config.getSite(argv.site), argv.concurrency);
  } else if (argv.file) {
