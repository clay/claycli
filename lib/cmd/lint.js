 ...
 */
function lintURL(uri, isRecursive) {
  let spinner;

  spinner = logger.startSpinner('Fetching component data...');
  clay.checkAllReferences(uri, isRecursive)
...
function handler(argv) {
  if (argv.url && clayUtils.isComponent(argv.url)) {
    return lintURL(argv.url, argv.recursive);
  } else if (argv.file) {
