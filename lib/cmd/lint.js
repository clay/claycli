...
function pushRestError(err, push) {
  push(null, { action: 'error', message: err.url }); // every url that errors out should be captured
}
      ...
      if (res instanceof Error) {
        ...
        return rest.get(url)
          ...
          .flatMap((data) => {
            ...
            return h([h.of({ action: 'success', message: url }), h(children)]).merge();
          })
        ...
        return h.of({ action: 'success', message: url });
      }
    });
}
      ...
      if (res instanceof Error) {
        ...
        return rest.get(url)
          ...
          .flatMap((data) => {
            ...
            return h([h.of({ action: 'success', message: url }), h.of(layout), h(children)]).merge();
          })
          .errors(pushRestError)
          .flatMap((uri) => checkComponent(uri, prefix))
          .ratelimit(concurrency, CONCURRENCY_TIME);
...
function checkPublicUrl(url, concurrency) {
  return rest.findURI(url)
    ...
    .flatMap(({ uri, prefix }) => {
      ...
      return h([h.of({ action: 'success', message: url }), checkPage(pageURL, prefix, concurrency)]).merge();
    }).errors(pushRestError);
}
...
function lintSchema(str) {
  return h([str])
    .map(yaml.safeLoad)
    .errors((e, push) => {
      push(null, { action: 'error', message: `YAML syntax error: ${e.message.slice(0, e.message.indexOf(':'))}` });
    }).flatMap((obj) => {
      if (obj.action && obj.action === 'error') {
        ...
        if (noDescription(obj)) {
          errors.push({ action: 'error', message: 'Schema has no _description' });
        }

        if (nonCamelCasedProps(obj).length) {
          errors.push({ action: 'error', message: 'Properties must be camelCased', details: nonCamelCasedProps(obj).join('\n') });
        }

        if (nonexistentGroupFields(obj).length) {
          errors.push({ action: 'error', message: 'Fields referenced by groups don\'t exist', details: nonexistentGroupFields(obj).join('\n') });
        }

        if (errors.length) {
          return h(errors);
        } else {
          return h.of({ action: 'success' });
        }
      }
    });
}
