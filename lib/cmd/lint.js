    ...
    .errors((e, push) => {
      ...
      if (obj.result && obj.result === 'error') {
        ...
        if (noDescription(obj)) {
          errors.push({ result: 'error', message: 'Schema has no _description' });
        }

        if (nonCamelCasedProps(obj).length) {
          errors.push({ result: 'error', message: 'Properties must be camelCased', example: nonCamelCasedProps(obj).join('\n') });
        }

        if (nonexistentGroupFields(obj).length) {
          errors.push({ result: 'error', message: 'Fields referenced by groups don\'t exist', example: nonexistentGroupFields(obj).join('\n') });
        }
