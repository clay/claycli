...
function lintFile(filepath, isRecursive) {
  ...
  return files.get(filepath, isRecursive).then((obj) => {
    ...
    // lint schemas
    missingDesc = _.compact(_.map(obj.schemas, (schema, filename) => {
    ...
    if (missingDesc.length) {
      logger.error(`${pluralize('schema', missingDesc.length, true)} ${missingDesc.length > 1 ? 'are' : 'is'} missing _description:\n`, _.map(missingDesc, (filename) => `- ${filename}`).join('\n') + '\n');
    } else if (_.size(obj.schemas)) {
      logger.success('All schema _descriptions exist!', `(${pluralize('schema', _.size(obj.schemas), true)})`);
    }
  }).catch((e) => {
