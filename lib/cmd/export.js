...
function exportSingleUrl(url, filename, argv) {
  let stream = clayInput.streamAssets(url, argv.concurrency)
...
function builder(yargs) {
  return yargs
    .usage('Usage: $0 export [--site, --url] [file]')
    .example('$0 export -u domain.com/components/foo', 'export component to stdout')
    .example('$0 export -u domain.com/components/foo backup.json', 'export component to json')
    .example('$0 export -u domain.com/pages/123 page-backup', 'export page to yaml')
...
function handler(argv) {
  ...
  if (argv.url) {
    return exportSingleUrl(config.normalizeSite(argv.url), filename, argv);
  } else if (argv.site) {
