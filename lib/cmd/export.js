...
function exportSingleItem(url) {
  return rest.get(url).map((res) => ({ [prefixes.urlToUri(url)]: res }));
}
...
function exportComponentInstances(url, prefix) {
  return rest.get(url).flatMap((res) => {
...
function exportSinglePage(url, prefix, includeLayout) {
  ...
  return rest.get(url).flatMap((res) => {
    ...
    return h([h.of(res), _.map(children, (uri) => exportSingleItem(`${prefixes.uriToUrl(prefix, uri)}.json`))]).flatten();
  });
}
...
function exportSingleURI(url) {
  return rest.get(url, { type: 'text' }).map((res) => ({ [prefixes.urlToUri(url)]: res }));
}
...
function fromURL(rawUrl, options = {}) {
  ...
  try {
    ...
    return h.fromError(new Error(`Unable to export ${url}: ${e.message}`));
  }

  if (options.yaml) {
    // return a single bootstrap
    stream = stream.flatMap((dispatch) => prefixes.remove(dispatch, prefix)).ratelimit(concurrency, CONCURRENCY_TIME);

    return formatting.toBootstrap(stream);
  } else {
    // return a stream of dispatches
    return stream.flatMap((dispatch) => prefixes.remove(dispatch, prefix)).ratelimit(concurrency, CONCURRENCY_TIME);
  }
}
