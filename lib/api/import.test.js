...
function mockPagesIndex(...pagePublishStates) {
  ...
  return clayInput.getPagesInSite.withArgs(aSite).returns(h(results));
}
...
describe('import api', function () {
  ...
  describe('importSite', function () {
    ...
    it ('calls importList with overwrite set to true if overwrite includes "lists"', function () {
      return fn(aSite, bSite, {overwrite: ['lists']})
        .collect()
        .toPromise(Promise)
        .then(() => {
  ...
  describe('importLists', function () {
    ...
    it ('calls importList with all relevant arguments and options for each list in site', function () {
      const mockOpts = {key: 'foo', concurrency: 1, headers: {foo: 'bar'}, overwrite: true};

      clayInput.getListsInSite.withArgs(aSite).returns(h(['authors', 'pages']));
