...
describe('import api', function () {
  ...
  describe('importPages', function () {
    ...
    it ('does not overwrite existing pages or components by default', function () {
      mockPagesIndex(false);
      mockReq('GET', 'http://a.com/pages/0', {main: ['a.com/components/foo/instances/0']});
      mockReq('GET', 'http://a.com/components/foo/instances/0.json', {});
      mockReq('GET', 'http://b.com/pages/0', {});
      mockReq('GET', 'http://b.com/components/foo/instances/0', {});
      ...
      return fn('http://a.com', 'http://b.com')
        ...
        .then((results) => {
          assertItems(results, [
            ...
            {url: 'http://b.com/components/foo/instances/0', status: 'skipped'},
          ]);
        });
    });
    ...
    it ('overwrites existing components if overwrite includes "components"', function () {
      mockPagesIndex(false);
      mockReq('GET', 'http://a.com/pages/0', {main: ['a.com/components/foo/instances/0']});
      mockReq('GET', 'http://a.com/components/foo/instances/0.json', {foo: 'bar'});
      mockReq('GET', 'http://b.com/pages/0', {});
      mockReq('GET', 'http://b.com/components/foo/instances/0', {});
      mockReq('PUT', 'http://b.com/pages/0', 200);
      mockReq('PUT', 'http://b.com/components/foo/instances/0', 200);

      return fn('http://a.com', 'http://b.com', {overwrite: ['pages', 'components']})
        .collect()
        .toPromise(Promise)
        .then((results) => {
          assertReq('PUT', 'http://b.com/pages/0', {main: ['b.com/components/foo/instances/0']});
          assertReq('PUT', 'http://b.com/components/foo/instances/0', {foo: 'bar'});
          assertItems(results, [
            {url: 'http://b.com/pages/0', status: 'success'},
            {url: 'http://b.com/components/foo/instances/0', status: 'success'},
          ]);
        });
    });
    ...
    it ('limits page imports if "limit" is set', function () {
      ...
      return fn('http://a.com', 'http://b.com', {limit: 2})
        .collect()
        .toPromise(Promise)
        .then(() => {
          clayInput.getPagesInSite.firstCall.args[1].limit === 2;
        });
    });
    ...
    it ('offsets pages imports if "offset" is set', function () {
      ...
      return fn('http://a.com', 'http://b.com', {offset: 2})
        .collect()
        .toPromise(Promise)
        .then(() => {
          clayInput.getPagesInSite.firstCall.args[1].offset === 2;
        });
    });
  ...
  describe('importLists', function () {
    ...
    beforeEach(function () {
      sandbox.stub(clayInput, 'getListsInSite');
      sandbox.stub(lib, 'importUrl');
      lib.importUrl.withArgs('http://a.com/lists/authors').returns(h(['a','b']));
      lib.importUrl.withArgs('http://a.com/lists/pages').returns(h(['c','d']));
    });

    it ('calls importList with all relevant arguments and options for each list in site', function () {
      const opts = {key: 'foo', concurrency: 1, headers: {foo: 'bar'}, overwrite: true};

      clayInput.getListsInSite.withArgs('http://a.com').returns(h(['authors', 'pages']));
      return fn('http://a.com', 'http://b.com', opts)
        .collect()
        .toPromise(Promise)
        .then((results) => {
          ...
          assertItems(results, ['a','b','c','d']);
          expect(authorsCall.args[0]).to.equal('http://a.com/lists/authors');
          expect(authorsCall.args[1]).to.equal('http://b.com');
          expect(authorsCall.args[2]).to.eql(opts);
          expect(pagesCall.args[0]).to.equal('http://a.com/lists/pages');
          expect(pagesCall.args[1]).to.equal('http://b.com');
          expect(pagesCall.args[2]).to.eql(opts);
        });
    });
