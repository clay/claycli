...
/**
* Assert that a request to the specified URL with the specified method has NOT been made.
* @param {string} method
* @param {string} url
**/
// function assertNoReq(method, url) {
//   const calls = fetch.send.getCalls(),
//     oneMatches = calls.some(call =>
//       call[0] === url &&
//       // true if "method" arg not set or if method matches
//       (!method || call.args[1].method === method)
//     );

//   expect(oneMatches).to.be.false;
// }
...
describe('import api', function () {
  ...
  describe('importUrl', function () {
    ...
    it ('if target site list exists, merge into it', function () {
      mockReq('GET', 'http://a.com/lists/a', ['a', 'b', 'c']);
    ...
    it ('overwrite lists if ovewrite includes "lists"', function () {
      mockReq('GET', 'http://a.com/lists/a', ['a', 'b', 'c']);
  ...
  describe('importPages', function () {
    ...
    it ('does not PUT to the same component twice', function () {
      ...
      return fn('http://a.com', 'http://b.com')
        ...
        .toPromise(Promise)
        .then(() => {
          matchReq('PUT', 'http://b.com/components/c/0', {baz: 'zar'}).length === 1;
        });
    });
