...
describe('import api', function () {
  ...
  describe('importPages', function () {
    ...
    it ('limit pages imported if opts.limit is set', function () {
      mockPagesIndex();
  ...
  describe('importFile', function () {
    ...
    it ('imports components from the specified file', function () {
      files.get.withArgs(mockFile).returns(h([
        {'/components/foo/instances/1': {a: 'b'}},
        {'/components/foo/instances/2': {c: 'd'}}
      ]));
      mockReq('PUT', `${bSite}/components/foo/instances/1`, 200);
      mockReq('PUT', `${bSite}/components/foo/instances/2`, 200);
      return fn(mockFile, bSite).collect().toPromise(Promise).then((results) => {
        assertItems(results, [
          {
            status: 'success',
            url: `${bSite}/components/foo/instances/1`
          }, {
            status: 'success',
            url: `${bSite}/components/foo/instances/2`
          }
        ]);
      });
    });
