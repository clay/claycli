...
* @return {Stream}
**/
function streamItemsForImport(url, prefix, {concurrency, key, headers, overwrite = 'notLayout'}) {
  ...
  switch (overwrite) {
    case 'none': // remove anything existing from the stream
      stream = stream.flatFilter(item => filterExisting(item, {concurrency, key, headers}));
      break;
    case 'all': // do nothing; keep everything in the stream
      break;
    default: // remove any existing layouts from the stream
      stream = stream.flatFilter(item => {
        if (!item.isLayout) return h.of(true);
        return filterExisting(item, {concurrency, key, headers});
      });
  }
 ...
 * @return {Stream}
 */
function filterExisting(item, {concurrency, headers, key}) {
...
function importPages(sourceSite, prefix, {limit, offset, concurrency, key, sourceKey, published, overwriteLayouts, headers} = {}) {
  ...
  return clayInput.getPagesInSite(sourceSite, {key: sourceKey, limit, offset})
    ...
    .flatMap(pageUrl => streamItemsForImport(pageUrl, prefix, {concurrency, key, overwriteLayouts}))
