...
function assertValidOverwrite(overwrite) {
  const acceptedTypes = ['lists', 'components', 'pages', 'layouts', 'users'],
    valid = overwrite === 'all' || overwrite === 'none' ||
      Array.isArray(overwrite) && overwrite.every(i => acceptedTypes.includes(i));

  if (!valid) throw new Error(`invalid "overwrite" value: ${overwrite}`);
  if (Array.isArray(overwrite) && overwrite.includes('layouts') && !overwrite.includes('components')) {
    throw new Error('Under-specified: If "overwrite" includes "layouts" it must also include "components"');
  }
}
