...
function streamAssetsForImport(sourceUrls, targetSite, {concurrency, overwriteLayouts} = {}) {
  ...
  return clayInput.streamAssets(sourceUrls, concurrency)
    ...
    // do not overwrite existing layouts or their children unless "overwriteLayouts" is set
    .flatMap(asset => overwriteLayouts ?
      h.of(asset) : // i.e. treat layouts like anything else
      skipExistingLayoutCmpts(asset, {concurrency})) // check for existing and skip
...
/**
* If the specified asset is a layout, check if it already exists.
* If it does, set the "skip" property to true.
* @param {Object} asset
* @param {Object} [options]
* @param {number} [options.concurrency]
* @return {Stream}
**/
function skipExistingLayoutCmpts(asset, {concurrency}) {
  return asset.isLayout ?
    h(atomizeAsset(asset)).flatMap(asset => checkExisting(asset, {concurrency})) :
    h.of(asset);
}

/**
* Split an asset with composed data into an array of "decomposed"
* assets that have no child data.
* @param {Object} asset
* @return {Object[]}
**/
...
/**
 * Import users from one site to another. Merge with existing lists unless is true.
 * @param {string} sourceSite Prefix of source site
 * @param {string} targetSite Prefix of target site
 * @param {object} [opts]
 * @param {string} [opts.key] Authorization key of target site
