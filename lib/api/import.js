...
* @param {Stream|string[]|string} stream or array or URLs, or single URL
...
* @return {Stream}
**/
function streamItemsForImport(stream, prefix, {concurrency, headers, overwrite} = {}) {
  // stream Clay assets ({url, data, isLayout}) for each url
  return clayInput.streamAssets(stream, concurrency)
 ...
 * @param {Stream|string[]|string} url Stream or array of URLs, or single URL
 * @param {string} prefix of target site
 * @param {object} [opts]
 * @param {number} [opts.concurrency]
 * @param {string} [opts.key] Authorization key for target site
...
function importUrl(url, prefix, {concurrency, key, headers, overwrite} = {}) {
  return streamItemsForImport(url, prefix, {concurrency, headers, overwrite})
...
function importPages(sourceSite, prefix, {limit, offset, concurrency, key, sourceKey, published, overwrite, headers} = {}) {
  return clayInput.streamPageUris(sourceSite, {key: sourceKey, limit, offset})
    // unless published is true, filter out all published URIs
    .filter(pageUri => published === true || clayUtils.getPageVersion(pageUri) !== 'published')
    .map(pageUri => urlUtil.uriToUrl(sourceSite, pageUri))
    .through(urlStream => importUrl(urlStream, prefix, {concurrency, key, overwrite, headers}));
}
