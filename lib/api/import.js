...
function importUrl(url, prefix, {concurrency, key, headers, overwriteLayouts}) {
  const isPublishedPage = clayUtils.isPage(url) && _.endsWith(url, '@published');
  ...
  // If the URL is a published page, you must import the page *after* its components
  // have been imported because the target site's publishing chain may look at them
  if (isPublishedPage) {
    let pageItem;

    return stream
      .doto(item => {
        if (item.isPage) pageItem = item; // remember page
      })
      .filter(item => !item.isPage) // remove page from stream
      .flatMap(item => rest.put(item, {key, concurrency, type: 'json', headers}))
      .collect() // wait for cmpt puts before continuing
      .flatMap((putResults) => h([ // add page PUT to stream
        h(putResults),
        rest.put(pageItem, {key, concurrency, type: 'json', headers})
      ]))
