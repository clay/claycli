 ...
 * @param {boolean} [opts.overwrite] Overwrite level
 * @return {Stream}
 */
function importUrl(url, prefix, {concurrency, key, headers, overwriteLayouts}) {
  let stream = streamItemsForImport(url, prefix, {concurrency, key, headers, overwriteLayouts});
...
* @return {Stream}
**/
function streamItemsForImport(prefix, {concurrency, headers, overwrite = 'notLayout'}) {
  return (s) => s
    .flatMap(url => clayInput.streamPageItems(url, concurrency))
    .uniqBy((a, b) => a.url === b.url) // drop any URLs we've already processed
    .flatMap(item => clayInput.composeItem(item, concurrency)) // get cmpt data
    .flatMap(chunks.replacePrefixes(prefix)) // replace all prefixes with target site's prefix
    .through(filterOverwrite(overwrite, {concurrency, headers}));
}

/**
* Based on the overwrite level, filter out some items from a stream of
...
*/
function filterOverwrite(overwrite, {concurrency, headers}) {
  return s => {
    switch (overwrite) {
      case 'none': // remove anything existing from the stream
        return s.flatFilter(item => filterExisting(item, {concurrency, headers}));
      case 'all': // do nothing; keep everything in the stream
        return s;
      default: // remove any existing layouts from the stream
