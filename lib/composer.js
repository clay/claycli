...
const _ = require('lodash'),
  ...
  log = require('./debug-logger')(__filename),
  refProp = '_ref';
...
function addComponent(item, bootstrap, added) {
  ...
  if (!data || !_.size(data)) {
    log.warn(`No data in the bootstrap for ${uri}`);
    return item; // just return the _ref, since it doesn't point to any data we currently have
    // note: there might already be data in the database, or elsewhere, so it's just a warning
  } else {
