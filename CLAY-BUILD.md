...
| `.clay/_view-init.js` | `generateViewInitEntry()` in `scripts.js` | Component mounting bootstrap. Replaces `_client-init.js`. Includes sticky-event shim. |
| `.clay/_kiln-edit-entry.js` | `generateKilnEditEntry()` in `scripts.js` | Aggregates all `kiln.js` plugins for edit mode. |
