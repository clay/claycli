...
If installed globally, call `clay` from the command line. Much like `git`, `claycli` is configured using a dotfile (`.clayconfig`) file in your home folder. In it you may specify references to api keys and urls / site prefixes that you use frequently. For urls and site prefixes, it will assume `http://` and port `80` unless you specify otherwise.
...
For smaller Clay installations (or, ironically, for very large teams where devs spend most of their time on individual sites), you may specify a default api key and site prefix by using the `CLAYCLI_DEFAULT_KEY` and `CLAYCLI_DEFAULT_URL` environment variables.
...
Each line of a _dispatch_ contains [composed data for a component](https://github.com/clay/amphora/blob/master/lib/routes/readme.md#component-data) (or page, user, list, etc) and any data for its child components. This means that each line will be sent as a [cascading PUT](https://github.com/clay/amphora/pull/73) to the Clay server, which is a highly efficient way of importing large amounts of data. Note that a _dispatch_ is not meant to be human-readable, and manually editing it is a very easy way to introduce data errors.

A _dispatch_ may be piped into and out of many `claycli` commands. Because they're a special format (rather than regular JSON files), the convention is to use the `.clay` extension, but this isn't required.

```
clay lint < article_dump.clay
clay import -k prod domain.com < article_dump.clay
```
...
clay config --url my-cool-article domain.com/_components/article/instances/123 # a specific url
```
...
Verify Clay data and schemas against standardized conventions. You may specify a url or pass files into stdin.

Linting a page, component, or user url will verify that the data for that url exists, and (for pages and components) will verify that all references to child components exist. The url may be an alias specified via `clay config`, or may be omitted in favor of `CLAYCLI_DEFAULT_URL`.
...
Imports data into Clay from `stdin`. Data may be in _dispatch_ or _bootstrap_ format. Site prefix may be an alias specified via `clay config`, or may be omitted in favor of `CLAYCLI_DEFAULT_URL`. Key may be an alias specified via `clay config`, or may be omitted in favor of `CLAYCLI_DEFAULT_KEY`.

The `publish` argument will trigger a publish of the pages and/or components you're importing. Note that the generated url of an imported page may be different than its original url.
...
```
clay export [--key <api key>] [--concurrency <number>] [--layouts] [--limit <number>] [--offset <number>] [--yaml] [--verbose] [url]
```

Exports data from Clay to `stdout`. Data may be in _dispatch_ or _bootstrap_ format. Url may be an alias specified via `clay config`, or may be omitted in favor of `CLAYCLI_DEFAULT_URL`.

If the url is a site prefix (e.g. not a specific page, public url, component instance, user, or list), `claycli` will query the built-in `pages` index to pull the latest 10 pages from the site. You may specify `limit` and `offset` arguments to change the amount / position of the query, which is useful for long-running exports that you want to pause and resume.

You may also pipe in a newline-separated stream of urls to export (and use `limit` / `offset` against those urls).

By default, layouts are not exported when exporting a page. This allows you to easily copy individual pages between sites and environments. To trigger layout exporting, please use the `layouts` argument.

Note that `key` is only required when querying the `pages` index. It is not needed when exporting individual pages, public urls, components, users, or lists. It may also be omitted in favor of `CLAYCLI_DEFAULT_KEY`.
